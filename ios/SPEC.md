...
- [ ] `ios/project.yml`, `Config/Server.xcconfig`, `Info.plist`, `PrivacyInfo.xcprivacy` exist and are valid YAML, plist and JSON respectively — asserted by a test in `packages/tracker` that parses them, checks the three purpose strings are non-empty English, the background mode is `location`, `WKAppBoundDomains` has exactly the server's host, and no key in either plist is a number that also appears in `config.ts` (I1, mechanically)
