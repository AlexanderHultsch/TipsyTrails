...
Withdrawal is the same field set back to `null`, from the web app's Settings screen (Section 8.6) or the shell's own Consent screen, and it stops background tracking on the next state change and at the latest on the next start.
...
`useSampleTracking` (`packages/web/src/tracking/useSampleTracking.ts`) keeps its interface — every screen that reads `SampleTrackingState` reads it unchanged — and gains a driver chosen once at mount: the existing `watchPosition` driver, or a shell driver that subscribes to the bridge. Under the shell driver:

- **No `watchPosition`, no queue, no flush, no wake lock** (I4). `lastPosition` is set from `position` events; `newBars`, `visitUpdates`, `tooFastToReveal`, `revealVersion` and `discoveryVersion` are set from `flush` events, so the fog refetch, the stamps and the banner react exactly as they do to the hook's own posts today; `queueDepth` and `behindDepth` come from `queue` and `flush`.
- **The GPS state** is `computeGpsStatus` over the last `position` as before, so `SPEC.md` 8.6's three states and `GPS_STALE_MS` mean the same thing.
- **The connection state** is `computeConnectionStatus` over `navigator.onLine` and `behind` as before.
- **The third icon changes meaning, and `SPEC.md` 8.6 is amended to say so** (14). In Safari it is _foreground tracking_ — active or paused, and paused is how phones work. In the shell it is _tracking_ with four states drawn from the `tracking` event: **on in the background** (ok), **on while open only** (degraded — When In Use, or consent not given, with the panel's words saying which and offering `openConsent`), **blocked** (bad — reduced accuracy, denied, services off, with the reason in words), **idle** (bad — not signed in, which the screen showing it can only reach while signed in, so in practice a session being ended underneath it). The shapes do not change (`SPEC.md` 8.6); the words in the panel do.
- The teleport mode (`SPEC.md` 9.3) is unchanged: while it stands the hook posts the teleported point through its own path regardless of driver, because the teleport is a server-side test fixture and not a position source.
...
`users` gains `background_tracking_consented_at INTEGER NULL` by migration, defaulting to `NULL` — no existing account and no new registration consents by default. `PATCH /api/settings` accepts `{ backgroundTracking: boolean }` beside `isAnonymous`, setting the column to the server's current second on `true` and to `NULL` on `false`. The `User` shape (`SPEC.md` 9.6) gains `backgroundTrackingConsentedAt: number | null`, so `GET /api/auth/me`, login, registration and `PATCH /api/settings` all carry it.
...
- [ ] Migration adds `background_tracking_consented_at`; `PATCH /api/settings` sets and clears it on the server's clock; `User` carries it on every route that returns a user; it is absent from the leaderboard, profile and admin-user shapes — asserted
...
| 9.2 | `POST /api/samples` gains `rejected`; `PATCH /api/settings` gains `backgroundTracking` |
| 9.6 | `User` gains `backgroundTrackingConsentedAt`; the `POST /api/samples` row gains `rejected` with 9.1's shape; the teleport row says it carries it too |
