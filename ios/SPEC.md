...
**Eight message types, one shared row.** `visitStarted` and `visitEnded` are two distinct types that share the row above because they get the same treatment from the shell; the table's row count is one lower than its type count for exactly that reason.
...
| D5 | The eight outbound messages, at the moments 8.2 fixes | as above |
