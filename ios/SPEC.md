...
| # | Does | Verification |
| --- | --- | --- |
| E1 | The route and the fake host (`packages/tracker/src/replay/route.ts`, `host.ts`, with their tests) — built; `app.inject` as `fetch` and loading the built bundle against a migrated database — not built, blocked on row 14 | `pnpm --filter @tipsytrails/tracker test` |
...
| A walk through Karlsruhe reveals the right cells, discovers the right bars, completes a dwelt visit, survives a dead spot, a relaunch, a 401 and a 429 — through the built bundle against the real API (13.2) | not yet; Step E is unblocked and next (Section 12) | — |
