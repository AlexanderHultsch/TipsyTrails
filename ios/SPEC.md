...
**Version:** 0.15
**Status:** Partly built. Steps A, B and F are built on this branch; Steps C and D are built on `main` and merged in here; Step E (the replay harness) and Step G (the walk) are open.
**Parent:** `SPEC.md` at the repository root, v1.64. This document does not replace it and cannot contradict it; Section 14 here lists the amendments the parent needs before this one is in force.
...
| I7 | **This branch writes only to `ios/` and `packages/tracker/`.** It reads `packages/shared` and never writes to it. Every change the app needs from `packages/web`, `packages/api`, `packages/shared` or a root file is written into the list for `main` (Section 12, "The list for `main`") and applied to `main` in a separate session, reviewed as website work — `main` is the website's baseline, and iOS work must not be able to change what a player of the website sees or what a developer of it has to keep green. `PARENT-CONTRACT.md` Section 0 names the two channels this permits. `main` may write, without an issue, exactly three things in this branch's tree: the `**Parent:**` pin in this file's front matter; Sections 1 and 3 of `PARENT-CONTRACT.md`, because `main` authors the surface it moves and the row that records a change made there; and a line in `packages/tracker/package.json` whose lockfile entry is a root file (row 11). Every other write into `ios/` or `packages/tracker/` that `main` needs is an issue labelled `for-ios`. This branch reviews Section 1's diff at every merge, and the merge's row in Section 3 says so. |
...
| `packages/tracker` | devDependencies | `@tipsytrails/api` `workspace:*`, `@types/node`, `vite` — the same major versions `packages/web` carries |
| root | devDependencies | unchanged |
| `ios/` | — | no manifest; Apple frameworks only |

**`@tipsytrails/api` `workspace:*` was absent from that row until `main`'s v1.60.** Section 13.2's replay harness needs it as a devDependency — the harness builds `buildApp` from `@tipsytrails/api` against an in-memory SQLite — but substep A2 never added it, because nothing used it at the time, and it was a root file, `pnpm-lock.yaml`, that I7 (Section 1) put out of reach of this branch. `main` added the devDependency and its lockfile entry at v1.60 (Section 12, "The list for `main`", row 11), and the merge `4c6caa2` carried it in here. Row 11's own wording had not foreseen everything the import needed to resolve at all: `packages/api/package.json` gained a `main`/`exports` entry point, and `packages/api/tsconfig.build.json` gained `"declaration": true`, both needed for a `.d.ts` to exist for `packages/tracker` to import against. A table that describes a manifest must match the manifest; that is the table's whole promise.
...
│   ├── WALK.md                      # the field form of the six walks (Section 13.3)
...
| `settingsUpdated` | the `PATCH /api/settings` that `requestSettingsUpdate` triggered has settled, once per request | continues the Consent screen's flow (11.2) |

**Nine message types, one shared row.** `visitStarted` and `visitEnded` are two distinct types that share the row above because they get the same treatment from the shell; the table's row count, eight, is one lower than its type count for exactly that reason.

**Payloads.** `ready`, `signedIn`, `signedOut`, `requestNotifications` and `openConsent` carry nothing beyond `{ type }`; the shell reads only the message name for these five. `openExternal` carries `{ type: 'openExternal', url: string }`, the URL to open in Safari (8.5). The remaining three are where a shape had to be chosen rather than found:
...
- **`settingsUpdated`** carries `{ type: 'settingsUpdated', backgroundTracking: boolean, ok: boolean, reason?: 'offline' | 'unauthenticated' | 'server' }` — `backgroundTracking` echoes the request so the shell can match reply to call; `ok` is whether the server answered 2xx; `reason` is present only when `ok` is `false`: `offline` for a transport failure (the request never got an HTTP status), `unauthenticated` for a 401, `server` for every other status (a 400 from the schema included — that is a protocol bug, and the page logs it as it does today).
...
**The teleport mode (`SPEC.md` 9.3) is decided, and under the shell the page posts no samples, ever.** O-I8 is closed (15): the tracker is the sole writer of `POST /api/samples` for the account, with no exception, so the teleport mode is inert under the shell. The shell driver ignores `TeleportMode`, `lastPosition`, `queueDepth`, the internal `behindDepth` and `postError` stay the tracker's, `trackingActive` stays the tracker's `tracking` state — nothing about a teleport touches the tracker — and the Admin screen's teleport control does not render under the shell, the mirror image of 8.6's row, which renders only under it. Why: two posters against one account make the server's `lastAccepted` alternate between two places and the speed gate refuse some of both, which is worse than no fixture; the fixture stays available where it always was, Safari on the same phone. The alternative weighed and rejected was a bridge message pausing the tracker for the duration — a tracker state, a Swift path and a page message for an admin-only mode that reaches no player.
...
The web client's response guard (`packages/web/src/api/response-guards.ts`) does not check the field, because no web screen reads it; the tracker's guard does. The admin teleport (`SPEC.md` 9.3) answers with the same body and therefore the same field, and there **all five** counts are always zero, each for its own reason (`SPEC.md` 9.6): the synthesised fix carries accuracy `0`, its timestamp is the batch's own `nowMs`, an out-of-box point is refused with a 422 before the batch runs, and the speed guard is off.
...
4. Only then, and only once the server has recorded the consent (`settingsUpdated` with `ok`, 8.2), the iOS prompt for Always (6.2).

The Always prompt is Apple's gate and is now downstream of the recorded consent, so a phone never holds Always for a consent the account does not hold.
...
**After the button that records consent,** the shell calls `requestSettingsUpdate(true)` (8.2). If the page returned `true`, the button becomes disabled and reads "Recording…" until `settingsUpdated` arrives: on `ok`, 10.1's step 4 (the Always prompt) follows and `start` is re-called (5.4); on not `ok`, the Always prompt is **not** shown, the checkbox stays checked so the player can tap again, and the screen shows one sentence for the reason — `offline`: "Your phone is offline. Try again when it is connected."; `unauthenticated`: "Sign in to the game first."; `server`: "The server could not record your consent. Try again." If the page did not return `true` — a page older than this rule, possible because the page is served live and the app is not — the shell behaves as before this version: it proceeds to the Always prompt and learns the state through the next `start`.

Withdrawing consent is a button on this screen, behind a confirmation that names what it stops and what it keeps (everything revealed so far), and its flow is symmetric: `requestSettingsUpdate(false)`, wait for the reply if the page promised one, re-call `start` on `ok`, and on not `ok` show the same sentence and change nothing else — the status rows above read the server's state through `start`, never the shell's own belief.

**There is deliberately no timeout and no new constant.** The page answers exactly once when the request settles, WebKit bounds a hung request itself, and a player who leaves this screen while the reply is pending loses nothing: the reply's only effect is the Always prompt and a re-called `start`, both of which the next visit to this screen or the next launch repeats.
...
`counters.ts` is in B1 and not last because the queue drops and counts in the same step; `visits.ts` precedes the state machine because profile selection reads the visit set. B3's fourth call is `GET /api/bars/:id`, not yet in this document when B3 was built; 7.6 says why. `computeBehindDepth` lives in `packages/shared/src/tracking.ts`, where `main` put it at v1.64 under I7 (Section 1); `useSampleTracking`'s `flush()` calls it at both call sites, and `tracker.ts`'s two flush call sites call the same export instead of `queue.ts` holding its own copy, so the two share one function rather than being kept in step by wording.
...
- [ ] The queue of 7.4: enqueue drops on accuracy and staleness with separate counters, the cap drops the oldest, `behind` matches `useSampleTracking`'s definition on the same scenario (the two share one function, `computeBehindDepth` in `packages/shared/src/tracking.ts`, so they cannot drift)
...
- [x] `POST /api/samples` and `POST /api/admin/teleport` answer with `rejected`; a test per gate sends a batch that fails only that gate and asserts the one count; the existing web response guard still passes on the new shape — `packages/api/src/routes/fog.test.ts`, `describe('the rejected counts')`, and `packages/api/src/routes/admin-teleport.test.ts`
- [x] Migration adds `background_tracking_consented_at`; `PATCH /api/settings` sets it to the server's current second on `backgroundTracking: true` and to `NULL` on `false`; `User` carries it on every route that returns a user; it is absent from the leaderboard, profile and admin-user shapes — asserted — `packages/api/src/db/migrate.test.ts`, `packages/api/src/routes/account.test.ts`, `packages/api/src/routes/admin.test.ts`, `packages/api/src/routes/leaderboard.test.ts`, `packages/api/src/routes/profile.test.ts`
- [x] `PATCH /api/settings` takes the partial body of 9.2, with a test per row of that section's table: `{ isAnonymous }` alone leaves consent untouched, `{ backgroundTracking }` alone leaves anonymity untouched, both together apply both, `{}` and an unknown-key-only body are 400, either key `null` or non-boolean is 400, unauthenticated is 401. The existing `PATCH /api/settings` tests pass unedited — a body that answered 200 before answers 200 now — `packages/api/src/routes/account.test.ts`, `describe('PATCH /api/settings — the partial body')`
- [x] `DELETE /api/account` takes it with the row (cascade already does; a test says so) — `packages/api/src/routes/account.test.ts`, `'takes the background-tracking consent record with the row'`

### Step D — The web app

**Not built on this branch**, for the same reason as Step C: Section 8 is entirely `packages/web`. Its substeps below are carried out on `main`, in a separate session from the list that follows Step G, and reviewed as website work; this step's Definition of Done is checked here once that work has been merged in. The table and the Definition of Done stay in place for the same reason Step C's do. It was built on `main` at v1.62–v1.63 and merged into this branch at `4c6caa2`.
...
| D5 | The nine outbound messages, at the moments 8.2 fixes | as above |
...
- [x] `useSampleTracking` under the shell driver: no `watchPosition`, no `fetch` to `/api/samples`, no wake lock — asserted by spies that must not be called; **each of the thirteen rows of 8.3's table has a test**, including `trackingActive` across `tracking` / `blocked` / `idle`, `postError` staying `null` through a run in which the tracker's flushes fail, and `queueDepth` and the internal `behindDepth` taken from the `queued` and `behind` of `queue` and `flush`; `SampleTrackingState` still has thirteen members, asserted against the type so a fourteenth cannot be added without this item failing; the existing screens' tests re-run against the shell driver — `packages/web/src/App.shelltracking.test.tsx`
- [x] The four counters start at nought on mount and are advanced only by a `flush` that arrives after the subscription: a late-mounting listener seeded with the bridge's replayed `flush` payload gets `newBars` and `visitUpdates` and stamps nothing, refetches nothing and re-reconciles nothing — asserted with `useBarStamps` and `useVisits` mounted against a bridge that already holds a payload — `packages/web/src/App.shelltracking.test.tsx`, `describe('the four counters (ios/SPEC.md 8.2, 8.3)')` and `describe('a late-mounting map against a bridge that already holds a payload')`
- [x] The indicator's third icon shows the four states of 8.3 with the stated words, keeps its shape, and its accessible name states the state — `packages/web/src/App.shellscreens.test.tsx`, `describe('the third icon under the shell (ios/SPEC.md 8.3, SPEC.md 8.6)')`
- [x] The push offer becomes `requestNotifications` under the shell; `usePushSubscription` reports `unsupported` there — `packages/web/src/App.shellscreens.test.tsx`, `describe('the push offer under the shell (ios/SPEC.md 8.4)')`
- [x] The Settings row of 8.6 renders under the shell and not in Safari; `/privacy` carries the section of 10.2 everywhere — `packages/web/src/App.shellscreens.test.tsx`, `describe('the Settings row (ios/SPEC.md 8.6)')`, and `packages/web/src/App.privacy.test.tsx`
- [x] `ready`, `signedIn`, `signedOut`, `visitStarted`, `visitEnded`, `requestNotifications`, `openConsent` are posted at the stated moments 8.2's table gives them — `signedOut` before the logout request, asserted by order — `packages/web/src/App.shell.test.tsx`, `describe('the messages at the moments 8.2 fixes')`, and `packages/web/src/App.shellscreens.test.tsx` for `requestNotifications` and `openConsent`. `openExternal`'s payload shape is asserted directly (`packages/web/src/App.shell.test.tsx`, `'posts openExternal with the URL to open in Safari'`), but no screen calls it yet — 8.5 has the shell's own `WKNavigationDelegate` cancel and reopen external navigations natively, so the "link leaves the app" moment is not currently exercised by a web-side test
...
| # | Does | Verification |
| --- | --- | --- |
| E1 | The route, the fake host, the bundle loader and the runner (`route.ts`, `host.ts`, `bundle.ts`, `runner.ts`) and the fake API (`fake-api.ts`), with their tests — built; `app.inject` as `fetch` against a migrated database — not built, blocked on row 14 | `pnpm --filter @tipsytrails/tracker test` |
| E2 | Scenarios 1–4 of 13.2 — not built; each needs `app.inject` against a migrated SQLite database, blocked on row 14 | as above |
| E3 | Scenarios 5–8 of 13.2 — 6, 7 and 8 built and pass, through the built bundle against `fake-api.ts` (`scenarios.test.ts`); scenario 5 needs `app.inject` against a migrated database and is not built, blocked on row 14 | as above |

**Was blocked; both blockers have cleared.** Step E was blocked twice, and neither was a decision this branch could make, so the owner deferred it rather than work around either.

1. **The API did not send `rejected`.** `POST /api/samples` answered with four fields; Section 9.1 specifies a fifth, and `packages/tracker/src/api.ts`'s guard requires it, so every flush a harness run made would have come back `invalidResponse`. This was Step C's work, which I7 (Section 1) moved to `main`; it cleared at `main`'s v1.60.
2. **I7 forbade the dependency the harness needs.** Section 3's dependency table had claimed `packages/tracker`'s devDependencies include `@tipsytrails/api` `workspace:*` "(the replay harness only, Section 13.2)"; they did not — substep A2 left it out because nothing used it at the time — and adding it rewrites `pnpm-lock.yaml`, a root file I7 puts out of reach of this branch. This cleared at `main`'s v1.60 too, in the same session as blocker 1 (row 11 of "The list for `main`", below).

Both cleared on `main` and are carried into this branch by the merge `4c6caa2`. **Step E is therefore the next thing this branch builds, and it is the last verification this repository can still produce before Step G's walk** — the one that would prove the built tracker bundle drives a real Fastify app to set real fog cells in real SQLite, proven nowhere else in this repository.

**A third blocker surfaced on 2026-09-06, found when the harness was about to be built.** `packages/api/src/index.ts` exports only `buildApp` and `loadEnv`, and its `exports` map allows only `.`; `openDatabase` (`packages/api/src/db/index.ts`) and `runMigrations` (`packages/api/src/db/migrate.ts`) are not exported, and `buildApp(env, db)` itself takes an already-open `better-sqlite3` database — so the harness has no way to open or migrate one, and `packages/tracker` may not take on a `better-sqlite3` dependency of its own to do it instead (I7). Row 14 of "The list for `main`" clears it: `main` widens `packages/api/src/index.ts`'s exports to include both, with nothing else in the API changing. Until it lands, Step E remains the next thing this branch builds and cannot start. The database-free half of E1 was built ahead of row 14 regardless: `packages/tracker/src/replay/` now holds the route, the fake host, the bundle loader (`bundle.ts`), the runner (`runner.ts`) and the fake API (`fake-api.ts`), and scenarios 6, 7 and 8 of 13.2 already run against that half, through the built bundle in a `node:vm` context (`scenarios.test.ts`) — so what row 14 unblocks is the database, not the harness that drives it.
...
| F5 | `settingsUpdated` (8.2): `Web/WebViewController.swift` routes the ninth message to the delegate and returns `requestSettingsUpdate`'s value through the completion handler; `Screens/ConsentScreen.swift` waits for the reply and shows the three sentences of 11.2; a third mechanical guard in `packages/tracker/src/ios-swift.test.ts` asserts the Swift `switch` carries exactly the nine types of 8.2's table — built | `pnpm --filter @tipsytrails/tracker test` for the guard, reading for the rest |
...
- [x] `describe('GUARD 3: Web/WebViewController.swift routes exactly the Page → shell message types of 8.2')` — `packages/tracker/src/ios-swift.test.ts`; the rest of F5 — `requestSettingsUpdate`'s completion handler in `Web/WebViewController.swift`, and `Screens/ConsentScreen.swift`'s reply flow and the three sentences of 11.2 — reviewed by reading (13.1)

### Step G — The walk

Section 13.3, run by the owner, with the report of 7.8 attached to the commit that closes the step.

Step G has no substeps: it is six walks, and the owner runs them. The walks themselves are written out as a field protocol in `ios/WALK.md`; Step G is run from that file, and the owner's reports fill in its tables.
...
| 7 | `packages/web/src/screens/Settings.tsx` | the shell-only "Background tracking" row (Step D's D4) | 8.6 |
| 8 | `packages/web/src/screens/Privacy.tsx` | the iPhone-app section of 10.2, rendered everywhere (Step D's D4) | 8.6, 10.2 |
| 9 | `packages/web/src/tracking/useSampleTracking.ts` | done on `main` at v1.64: the function lives in `packages/shared/src/tracking.ts`, and `useSampleTracking.ts` calls it at both `flush()` call sites. The tracker adopted the same export and deleted its own copy at v0.9 of this document, so one implementation exists | 8.3, `PARENT-CONTRACT` D2 |
...
| 12 | `packages/web/src/shell/bridge.ts`, `packages/web/src/shell/useShellSettingsUpdate.ts`, `packages/web/src/shell/messages.ts` | the page's `requestSettingsUpdate` handler returns `true`, and when its `PATCH /api/settings` settles it posts `settingsUpdated` with the payload of 8.2 exactly once; `messages.ts` gains the ninth type and `SHELL_MESSAGE_TYPES` grows with it; the existing console logging of a failure stays | 8.2, 11.2 |
| 13 | `packages/web/src/tracking/useSampleTracking.ts`, `packages/web/src/screens/Admin.tsx` | the shell driver ignores `TeleportMode` and never posts; the teleport control does not render under the shell; the parent `SPEC.md` 7.2's closing sentence, which `main` scoped naming O-I8, can then say the web app posts nothing under the shell, without exception | 8.3, 15 (O-I8) |
| 14 | `packages/api/src/index.ts` | also export `openDatabase` and `runMigrations` (and their parameter types as needed), so the replay harness can open a temporary SQLite file and run `packages/api/migrations` against it before calling `buildApp`; nothing else in the API changes, and the entry point still calls nothing on import (the rule `package-entry.test.ts` states) | 13.2, Step E |
...
| Date | Issue | What |
| --- | --- | --- |
| 2026-09-13 | [#3](https://github.com/AlexanderHultsch/TipsyTrails/issues/3) | `main`'s commit `65b717e` fails `pnpm test` — `SPEC.md` reads v1.59 there while `HANDOVER.md`'s spec-version row still reads 1.58, which `packages/shared/src/spec-version.test.ts` binds together. `f18f93a`, the next commit, repairs it. Not a defect in any shipped code: a wrinkle in published history, so a `git bisect` crossing it should skip that commit. The issue names the two ways to close it, and the cheap one is a note in `HANDOVER.md`. |
...
| A walk through Karlsruhe reveals the right cells, discovers the right bars, completes a dwelt visit, survives a dead spot, a relaunch, a 401 and a 429 — through the built bundle against the real API (13.2) | Scenarios 6, 7 and 8 of 13.2, through the built bundle against the stand-in `fake-api.ts` — Vitest, `scenarios.test.ts` (Section 12); scenarios 1–5, which need the real API and a migrated database, are not yet — blocked on row 14 | — |
...
`packages/tracker/src/replay/bundle.ts` builds the tracker with Vite and evaluates the built IIFE in a `node:vm` context whose only global is `__tipsyTrailsHost`, exactly as Section 7.2 has the shell install it — `runner.ts`'s `startReplay`, `walk`, `settle` and `advanceSettled` drive it. Scenarios 6, 7 and 8 need no database — their assertions are on the tracker's own requests, state and emitted events — and run today, in `scenarios.test.ts`, against `fake-api.ts`: guard-valid answers to the four calls `api.ts` makes, and explicitly not the real API. Scenarios 1 to 5 assert against SQLite through the real Fastify app and are not yet built; they wait on row 14 (Section 12), the export that lets the harness open and migrate a database at all. Say this plainly: a scenario run against the stand-in proves the tracker and the built bundle, and proves nothing about the server — which is exactly what scenarios 1 to 5 are for.
...
| O-I4 | **A written DPIA** for background tracking (10.1). A draft now exists, written against this document's v0.13 and the parent `SPEC.md`'s v1.64, and handed to the owner outside the repository, as this item's own "kept outside the repository" rule requires; it carries ten points only the controller can settle. | Drafted; open until the controller completes and signs it |
| O-I5 | **Whether force-quit is relaunched** on current iOS (6.4). `ios/WALK.md`'s walk 3 (Force-quit) answers it. | Open until 13.3 |
| O-I6 | **The web view's Service Worker under App-Bound Domains** has never been seen running; Step F's first run is where it is confirmed. If it does not register, the offline shell is absent in the app and the Unreachable screen (11.4) covers more cases than it should. `ios/WALK.md`'s walk 1 (Baseline) — the first sustained run of the map screen — is where it is asked. | Open until Step F |
| O-I7 | **Battery.** No figure until walk 2 and the evening note of 13.3. If the cost is unacceptable, the levers are `TRACKER_WALKING_DISTANCE_FILTER_M` (a fix per cell instead of per half-cell, at the cost of the corner of a diagonal walk) and `TRACKER_DESIRED_ACCURACY_M` (a hundred metres is still under `FOG_MAX_ACCURACY_M`, and Core Location's hundred-metre mode uses far less radio). Both are `config.ts` changes and both are covered by scenario 2 of 13.2, which would show what each costs in cells. `ios/WALK.md`'s walks 2 and 5, and its evening-note item, capture it. | Open until 13.3 |
| O-I8 | **Two posters while an admin is teleported inside the shell** (8.3). Decided: under the shell the page posts no samples, ever, and the tracker is the sole writer of `POST /api/samples` for the account; the teleport mode is inert under the shell and the fixture stays available in Safari, where it always was (8.3). | Decided (v0.11); `main` builds it, row 13 |

---

## 16. Changelog

- **v0.15** — The first entry in **Section 12**'s "Found while building the app" table, which has been empty since v0.5 defined it. Reviewing `main`'s history from this branch turned up one commit, `65b717e`, whose tree fails `pnpm test`: it carries `SPEC.md` v1.59 while `HANDOVER.md`'s spec-version row still reads 1.58, and `packages/shared/src/spec-version.test.ts` binds the two. The next commit repairs it, so nothing shipped is wrong and nothing on this branch changes; what the row and its issue record is that a `git bisect` crossing that commit should skip it, and that closing it properly is either a note in `HANDOVER.md` or a history rewrite the owner must authorise. Filed as issue #3 with the `from-ios` label at the moment it was found, as `PARENT-CONTRACT.md` Section 0 requires, with the row and the issue carrying the same words.
- **v0.14** — `ios/WALK.md` is added, and the second half of E1 plus the three database-free scenarios of 13.2 are built. **`ios/WALK.md`** turns Section 13.3's table into six sections to run, one per walk, each with its preconditions as a checklist, the steps, when to export the Diagnostics report of 11.3 (per 7.8), 13.3's own "the report must show" / "the map must show" quoted, and an empty table of what was observed; a closing section holds the three notes 13.3 asks for beside the table. **Section 4.2**'s tree gains it. **`packages/tracker/src/replay/bundle.ts`** builds the tracker with Vite (`write: false`, cached across scenarios, no prior `pnpm build` needed) and evaluates the IIFE in a `node:vm` context whose only global is `__tipsyTrailsHost`, so a scenario drives the same bytes the shell would evaluate rather than `../tracker.js` from source. **`runner.ts`** adds `startReplay`, `walk`, `settle` and `advanceSettled`: `settle` yields one macrotask turn so the tracker's chained `await`s (a flush's `postSamples`, `start`'s sequential calls) resolve between one timer and the next, which a synchronous fake clock cannot do on its own; `advanceSettled` fires each due timer one at a time with a `settle` between, and `walk` steps a route through both. **`host.ts`** gains `fireNextDueTimer`, the single-timer sibling of `advanceTo`'s whole-sweep firing, for exactly this reason. **`fake-api.ts`** answers the four calls `api.ts` makes with guard-valid bodies, script-overridable per call, rebuilding `tracker.test.ts`'s `Scripted`/`fakeHost` idiom as a Vitest-free export — explicitly not the real API, and not to be confused with the `app.inject` half of E1 that still waits on row 14. **`scenarios.test.ts`** builds scenarios 6, 7 and 8 of 13.2 against all of the above, each asserting both sides of its boundary: 6, blocked while `reducedAccuracy` stands with zero requests meanwhile, resumed once it lifts; 7, exactly one `sessionLost` and no further request whatever fixes arrive after; 8, the retried flush lands at exactly `Retry-After` and the backoff is reset for the flush after that. **Section 12**'s Step E table is rewritten so E1, E2 and E3 each say what is built and what still waits on row 14, and the third-blocker paragraph's closing sentence now names the pieces built and the three scenarios already run against them. **Section 13.1**'s replay-harness row and **Section 13.2** (a new paragraph after the eight scenarios) both say the same thing in their own registers: scenarios 6–8 are proven in this repository, through the built bundle against the stand-in API, and scenarios 1–5, which need SQLite through the real Fastify app, are not. **Step G** gains a sentence pointing at `ios/WALK.md` as the file the step is run from. **Section 15** — O-I4 is now "Drafted; open until the controller completes and signs it": a draft DPIA exists, written against this document's v0.13 and the parent `SPEC.md`'s v1.64, handed to the owner outside the repository as the item itself requires, carrying ten points only the controller can settle; O-I5, O-I6 and O-I7 each gain a pointer to the walk in `ios/WALK.md` that answers them (walk 3, walk 1, and walks 2 and 5 with the evening note, respectively), their statuses unchanged. Neither Step E's nor Step G's Definition of Done gains a tick: Step E's still names all eight scenarios against `buildApp`, and Step G's still needs the walks themselves. The parent pin stays v1.64; nothing on `main` moved.
- **v0.13** — Step E's substep E1 splits into two halves, and the database-free one is built. **`packages/tracker/src/replay/route.ts`** builds the synthetic route of 13.2: `loadSeedBars`/`pickRouteBars` reads `data/seed/karlsruhe/bars.json` and returns the first pair, in file order, 900–1100 m apart by `haversineDistanceM`, deterministically and with no id hard-coded; `buildRoute` walks a straight line between two points at a given speed, one fix per second, timestamps `startMs + i * 1000` ms, accuracy jittered between 5 and 25 m by `seededRandom` (mulberry32, written out rather than a dependency); `applyDistanceFilter` reproduces Core Location's own `distanceFilter` for scenario 2, keeping the first fix and every fix after it at least the given distance from the last one kept. The 900–1100 m pair, the 5–25 m jitter and 13.2's 1.3 m/s (`WALKING_SPEED_MPS`) are this harness's own fixture parameters, not `config.ts` material — they pick and shape one synthetic test route rather than a threshold the tracker's own logic applies — so `CLAUDE.md`'s two-module rule does not reach them, and they stay named consts in `route.ts` with a comment saying so, the same place `packages/tracker/src/ios-swift.test.ts` keeps its own test-only tolerances. **`packages/tracker/src/replay/host.ts`** builds `createReplayHost`, a reusable `Host` (7.2) with a fake clock — `advanceTo`/`advanceBy` fire every due timer in due-time order, including one a firing timer schedules for a time at or before that same call's target, and refuse to move the clock backwards — a swappable `fetch` (`setFetch`) recording every request and its response or thrown error, and every other call `fakeHost` (`packages/tracker/src/tracker.test.ts`) already records, under the same names. **`route.test.ts`** and **`host.test.ts`** beside them. Not built: the other half of E1, `app.inject` as `fetch` and loading the built bundle against a migrated database, both blocked on row 14 of "The list for `main`" until `packages/api/src/index.ts` exports `openDatabase` and `runMigrations`. Step E's Definition of Done stays unticked. The parent pin stays v1.64; nothing on `main` moved.
- **v0.12** — Step F's substep F5 is built. **`Web/WebViewController.swift`** routes `settingsUpdated` (8.2's ninth message type, in a `switch` whose own comment now says "eight rows across nine message types" rather than "seven rows") to a new `WebBridgeDelegate` method, `webBridgeSettingsUpdated(backgroundTracking:ok:reason:)`, reading `backgroundTracking`, `ok` and the optional `reason` off the message body and ignoring a malformed one rather than crashing, following `handleOpenExternal`/`handleVisitMessage`'s own guard-and-return idiom. `requestSettingsUpdate(backgroundTracking:completion:)` moves into this same file from `Screens/ConsentScreen.swift`'s extension — F3's write scope, the reason it lived there, is gone now that F5 writes both files — and gains the second half of 8.2's contract: it reads `evaluateJavaScript`'s completion handler and reports to `completion`, on the main queue, whether the page returned exactly `true`. **`Screens/ConsentScreen.swift`** gains `ConsentReplyObserver`, an `ObservableObject` whose `Phase` (`idle` / `recording` / `failed` / `succeeded`, each carrying the `backgroundTracking` value it is about) tracks one request through to its reply, ignoring and logging a reply that arrives with nothing pending or for the other `backgroundTracking` value. `recordConsent` and `withdrawConsent` call the new `requestSettingsUpdate`; when the page promises a reply they call `begin` and wait, and `.onChange(of: consentReply.phase)` on the screen's root view runs the Always prompt or the withdrawal's `hasCheckedConsentSentence = false` only once `settingsUpdated` actually answers `ok`. A page that returns anything but `true` gets the pre-v0.11 behaviour unchanged, named in a comment as 11.2's own "page older than this rule" case. The consent button reads "Recording…" and disables while a reply is pending; a `.failed` phase shows, in `consentSection`'s existing footer, the one sentence 11.2 gives for `offline` / `unauthenticated` / `server`, an unknown or missing reason falling to the `server` sentence, and the checkbox is left exactly as it was. **`App/TipsyTrailsApp.swift`** has the app delegate own one `ConsentReplyObserver` beside its existing `TrackerStateObserver`, thread it through `RootView` to `ConsentScreen` as an `@ObservedObject`, and give `AppWebBridgeDelegate` the new protocol method, forwarding straight to `resolve`. **A third mechanical guard**, `GUARD 3` in `packages/tracker/src/ios-swift.test.ts`, parses `ios/SPEC.md`'s own "Page → shell" table in 8.2 for its nine backticked message types (a brace-counting reader rather than Guard 2's fixed-indentation one, so a reindent of the `switch` cannot silently break it) and asserts the Swift `switch` routes exactly those and no others. **Section 12** ticks Step F's table row for F5 and its Definition-of-Done item, naming `GUARD 3`'s `describe` verbatim and "reviewed by reading" for the rest, per 13.1. The parent pin stays v1.64; nothing on `main` moved, and nothing in `packages/tracker` changed outside this one test file.
- **v0.11** — Three decisions the owner has taken, written into this document, plus one dependency gap found while scoping Step E. **`requestSettingsUpdate` gets a reply.** **Section 8.2** — the call now returns `true` when the page has taken the request and will answer with the new `settingsUpdated` message, and `undefined` from a page that predates the reply; the shell reads the value through `evaluateJavaScript`'s completion handler, still reads consent's *state* only through `start`, and reads the *outcome of its own write* from `settingsUpdated`. The "Page → shell" table gains that row, with its own payload — `{ backgroundTracking, ok, reason? }`, `reason` one of `offline` / `unauthenticated` / `server` — and the section now counts nine message types across eight rows, one shared. **Section 5.4** gains the same "outcome from `settingsUpdated`, state from `start`" sentence. **Section 10.1** step 4 becomes conditional on `settingsUpdated`'s `ok`, with a sentence saying why: the Always prompt is Apple's gate and is now downstream of the recorded consent. **Section 11.2** gains the Consent screen's flow around the reply — the button disabled and reading "Recording…" while it waits, the three per-reason sentences on a not-`ok` answer, the fallback to the old behaviour when the page returns nothing, withdrawal's symmetric flow — and states that there is deliberately no timeout and no new constant: WebKit bounds a hung request itself, and a player who leaves the screen loses nothing. **Section 12** gains Step F's substep F5 and an unticked Definition-of-Done item naming `GUARD 3`, and "the list for `main`" gains row 12 for the page-side implementation. **O-I8 is closed: the page never posts under the shell.** **Section 8.3**'s teleport paragraph is rewritten: under the shell the page posts no samples, ever, the tracker is the sole writer of `POST /api/samples`, and the teleport control does not render there — the mirror image of 8.6's row. **Section 15**'s O-I8 row is marked "Decided (v0.11); `main` builds it, row 13" rather than deleted. "The list for `main`" gains row 13. **I7 says what `main` may write here.** **Section 1**'s I7 row now names the exactly three things `main` may write into this branch's tree without an issue — the `**Parent:**` pin, `PARENT-CONTRACT.md` Sections 1 and 3, and the `packages/tracker/package.json` line of row 11 — and says this branch reviews Section 1's diff at every merge. `ios/PARENT-CONTRACT.md` Section 0's "Keeping this file true" paragraph states the same rule in its own terms. **A third blocker on Step E, found scoping the harness.** `packages/api/src/index.ts` exports neither `openDatabase` nor `runMigrations`, and `buildApp(env, db)` takes an already-open database, so the replay harness of 13.2 has no way to open or migrate one without a `better-sqlite3` dependency of its own, which I7 forbids. **Section 12**'s Step E gains a paragraph recording this, and "the list for `main`" gains row 14: `packages/api/src/index.ts` exports both, nothing else in the API changing. Step E's Definition of Done stays unticked. The parent pin stays v1.64; nothing on `main` moved by this pass, and nothing in `packages/tracker` or the Swift changed — every substep this bullet names is `not built`.
- **v0.10** — This pass brings the document's account of Steps C and D, and of the server and web-app facts they rest on, in step with `main`'s v1.60–v1.64, merged in at `4c6caa2`. **Front matter** — `Status` now says Steps A, B and F are built on this branch, Steps C and D are built on `main` and merged in, and Steps E and G are open, in place of the v0.1 "nothing built" sentence. **Section 3** — the dependency table's `packages/tracker` devDependencies row now names `@tipsytrails/api` `workspace:*` beside `@types/node` and `vite`, matching the manifest since `main`'s v1.60; the paragraph below is rewritten as history, naming the two things row 11 had not foreseen — `packages/api` gaining a `main`/`exports` entry point and `"declaration": true` in `tsconfig.build.json` — that the import needed to resolve at all. **Section 9.1** — the teleport sentence is corrected from "`tooFast` always zero" to all five counts always zero, each for its own reason, mirroring `SPEC.md` 9.6's teleport row. **Section 8.2** — a sentence states plainly that the "Page → shell" table has eight message types across seven rows, `visitStarted`/`visitEnded` sharing one; the list-for-`main` row 3 that referred to "the seven message types" now says eight. **Section 8.3** — the third icon's `blocked` state now lists all four `BlockedReason` values, `restricted` included, matching `packages/tracker/src/events.ts`. **Section 5.4** — the withdrawal paragraph is scoped to match Section 8.6, built on `main`: the Settings row only opens the Consent screen, and the write happens only from that screen's two places. **Section 12** — Step C's and Step D's paragraphs each gain a sentence naming the `main` versions they were built at and the merge that carried them in; each step's Definition of Done items are ticked `[x]` where a test in the merged tree proves them, with the test file named per item, and Step D's D5 substep is corrected from "five" to "eight" outbound messages; "the list for `main`"'s row 7 citation is corrected from "Step D's D4/D5" to "Step D's D4"; Step E's "Deferred" paragraph is rewritten in the past tense — both blockers cleared at `main`'s v1.60, and Step E is now the next thing this branch builds. **Section 13.1** — the replay-harness row now says Step E is unblocked and next, rather than deferred. **`packages/tracker/src/events.ts`** — the comment above `SamplesResponse` is corrected: the web mirror has carried `rejected` since parent v1.60. The parent pin stays v1.64; nothing on `main` moved by this pass.
- **v0.9** — The tracker adopts `computeBehindDepth` from `@tipsytrails/shared` (`packages/shared/src/tracking.ts`, where `main` put it at v1.64 because `packages/web` cannot import `packages/tracker`) and deletes its own copy from `queue.ts`, so one implementation of Section 8.6's "behind" rule exists. The four cases of the tracker's `queue.test.ts` were already asserted in `packages/shared/src/tracking.test.ts` and are removed with the copy. **Section 12**'s paragraph under Step B's substep table, Step B's Definition of Done, and "The list for `main`"'s row 9 are corrected to say the two share one function rather than being kept in step by wording. The parent pin is v1.64, carried in by the merge `4c6caa2`; `PARENT-CONTRACT.md` Section 3 has that merge's row.
