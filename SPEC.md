...
A teleport is not a way for **this client** to sample from two places at once: the web app's watch is not started, so the phone's real position is not watched, queued or posted by it while the mode is on, and the mode ends with an explicit request (Section 9.3) rather than with the next fix.

**Inside the iPhone app the native tracker goes on sampling regardless**, and that is a second poster rather than a second watch in this client. The teleport is an admin fixture belonging to the web app, and it does not reach the tracker: the account therefore has two posters for as long as an admin stays teleported inside the app, and the server's previous-accepted position alternates between the two, refusing some of both on step 4. `ios/SPEC.md` O-I8 records it, names the two ways out — a bridge message pausing the tracker for the duration, or accepting the interleaving as the cost of a fixture — and says why neither is applied: the mode is admin-only, needs `ADMIN_TELEPORT_ENABLED` (Section 4.3), and reaches no player.
