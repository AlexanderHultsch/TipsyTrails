...
| What is every number, and where does it live?               | 7.1 — `config.ts`, or `server-config.ts` if a client may not have it |
...
3. All constants defined in this document must live in one of exactly **two** config modules, never inlined at call sites. This includes rate limits, radii, thresholds, tolerances, and timeouts. The two are `packages/shared/src/config.ts`, which is client-safe and is bundled into the browser, and `packages/shared/src/server-config.ts`, which `packages/web` cannot import. One question decides which: a constant a client must not be given goes in `server-config.ts`, and every other constant stays in `config.ts`. There is no third module and no constant is in both. (Until v1.54 there was one module, and the badge floors of Section 7.7 shipped to the browser inside it.)
...
| `pnpm typecheck` | `tsc --noEmit` in every package, after a `pretypecheck` build of `shared` and then `api` |
| `pnpm lint` | `eslint .` across the workspace |
| `pnpm format:check` | `prettier --check .` — note that `SPEC.md` and `README.md` are in `.prettierignore` and are **not** checked |
| `pnpm test` | `vitest run` in every package, after a `pretest` build of `shared` and then `api` |
...
**`packages/api` is a package other packages import, and not only a server this repository runs.** Its manifest names an entry point — `main`, `types` and an `exports` block, all pointing at `dist/index.js`/`dist/index.d.ts` — and `tsconfig.build.json` emits declarations, exactly as `packages/shared` does. `src/index.ts` is that entry and exports `buildApp` and `loadEnv`; `src/server.ts` is the one that listens and is deliberately not what the package name resolves to, so importing this package starts nothing. `packages/tracker` takes a devDependency on it for `ios/SPEC.md` 13.2's replay harness, which reaches the real API through `import { buildApp } from '@tipsytrails/api'`. Before v1.60 the package declared neither `main` nor `exports`, so that import resolved to a directory with no entry point and failed with `ERR_MODULE_NOT_FOUND` — at resolution, which no `tsc --noEmit` looks at, so `packages/api/src/package-entry.test.ts` is what holds all of it: the entry point, the declarations, and the workspace link resolving from `packages/tracker`'s own position.

**`packages/api` and `packages/web` resolve `@tipsytrails/shared` through a gitignored `dist`, and `packages/tracker` resolves `@tipsytrails/api` through another, so both builds have to run before anything reads them.** The root `package.json` runs the pair as `pretest` and `pretypecheck` as well as `prepare`, `shared` first because `api`'s build reads it. Without that, a signature-preserving change to a shared rule leaves `pnpm typecheck` at zero errors and the suite green, because both were reading the *previous* build off disk.
...
│   ├── seed/
│   │   └── karlsruhe/             # committed: bars.json, city.geojson, districts.geojson, neighbours.geojson, grid.bin, grid-meta.json
...
└── scripts/
    ...
    ├── collapse-seed-duplicates.ts # re-apply 11.1's collapse to a committed bars.json, offline
...
**On the standalone path the API's port is resolved by Compose, not by each container for itself.** `docker-compose.yml` writes `${API_PORT:-${PORT:-3000}}` once, in a top-level anchor — the same precedence and the same empty-is-unset rule as `loadEnv` — and hands that one value to both services: to the api container as `API_PORT`, which is also what its healthcheck probes, and to the caddy container, where it is the `{$API_PORT:3000}` substitution `caddy/Caddyfile`'s `reverse_proxy` line reads. It has to be one value and not two, because Caddy resolves that substitution inside its own container when it loads its config and cannot ask the app which port it chose. A caddy service without it silently uses the fallback written into the Caddyfile instead, which is what shipped until v1.56: any `API_PORT` but `3000` left the API listening on one port and Caddy proxying to another, every `/api/*` request answering 502, and the api service still reporting healthy because its healthcheck did read the variable. `packages/api/src/compose-caddy-env.test.ts` asserts the general form of that rule — every variable `caddy/Caddyfile` references is named in the caddy service's `environment` — rather than the one variable, because the next one added to that file would fail the same silent way. The caddy service is given exactly those variables and not `env_file: .env`, which would also hand the public-facing container the `SESSION_SECRET`, `ADMIN_PASSWORD` and `VAPID_PRIVATE_KEY` in that file.
...
What the pasted `Caddyfile` excerpt covers is one site's `handle` block and the 404 fallback, and nothing else, so it settles none of the file's global options. Whether `auto_https` is off, and whether `trusted_proxies` or a `header_up` override for `X-Forwarded-For` is configured, are open questions against this evidence; earlier versions of this section asserted all three, on second-hand information. None of that is load-bearing any more: since v1.46 no rate limit is keyed on an address, `trustProxy` is a list of private ranges rather than a hop count, and what it decides is the address in Fastify's log lines and nothing else (Section 9.4).
...
**`background_tracking_consented_at` exists for a client this app does not otherwise have.** Added by migration `004_users_background_tracking_consent.sql`, nullable with no default so no existing account and no new registration consents by default, it records when a player last gave the separate consent the iPhone app's background tracking needs (`ios/SPEC.md` 10.1) — the record Article 7(1) GDPR wants a controller to be able to produce. `PATCH /api/settings` (Section 9.2) is the one route that writes it, `true` re-stamping the server's current second and `false` clearing it back to `NULL`; nothing in this browser-only server ever writes it on its own, and nothing here reads it either — the column exists to be read by the iPhone shell at start (`ios/SPEC.md` 5.4, 7.3), and it is deleted with the account like every other column here (Section 10.6). It reaches a client only on the `User` of Section 9.6 — every route that answers with a user carries it, and the three shapes that describe a user without being one, the leaderboard entry, `GET /api/profile/:handle` and the admin user list, deliberately do not: none of them has a reason to know when somebody consented, and each absence is asserted rather than left to hold by accident.
...
All of these live in **two** modules under `packages/shared/src/` — `config.ts` and, since v1.54, `server-config.ts` — and those files are authoritative: the block below reproduces every key they export, with the files' own comments reduced — one trailing line per key, or a short header of a line or three where one comment introduces a group of keys and there is no single key to hang it on. A comment here that is not shorter than the one in the source file is a copy rather than a reduction, and copies are what go stale; the reasoning belongs in the source file and in the section that owns the constant. A constant named anywhere else in this document is defined here or nowhere.

**Why the two modules are one block here and not two.** `server-config.ts` holds the constants a client may not be given (Section 0, rule 3), and reproducing it in a fenced block of its own, elsewhere in this section, would let a reader of either block forget the other — which is exactly the failure this split repaired, a constant nobody remembered was reaching the browser. So the split is drawn inside the block, as the file boundary it is, and this section keeps its one promise: every constant, in one place, in the order the files are read. A reader adding a constant sees both halves at once, which is what rule 3's question needs.

```ts
// packages/shared/src/config.ts — client-safe; every key here is bundled into
// the browser, because packages/web imports CONFIG as a value in twelve modules.
export const CONFIG = {
  ...
  IMPORT_DUPLICATE_RADIUS_M: 15,  // import-side collapse radius, see 11.1 — tighter than the submission radius on purpose
  ...
  BADGE_KINDS: ['explorer', 'barfly'],  // the client-safe half of 7.7's badges; an array because the order is the shelf's
...
// packages/shared/src/server-config.ts — the constants a client may not be
// given. Reached only through the `@tipsytrails/shared/server` subpath, which
// `index.ts` does not re-export, ESLint forbids inside packages/web, and
// packages/web/src/bundle.test.ts proves absent from the built bundle.
// `satisfies Record<BadgeKind, …>` over CONFIG.BADGE_KINDS is what stops the
// kinds and their floors drifting apart now that they live in two files.
...
} as const satisfies { BADGE_THRESHOLDS: Record<BadgeKind, Record<BadgePeriod, number>> };
```

### 7.2 Position sampling

**In a browser** the client obtains positions via `navigator.geolocation.watchPosition({ enableHighAccuracy: true })` while the app is in the foreground, and holds a Screen Wake Lock while an active exploration session is running. **Inside the iPhone app it does none of that.** The same hook takes a second driver there (`ios/SPEC.md` 8.3): no watch, no wake lock, no client-side queue and no post of its own, because the native tracker is the only sampler in the app (`ios/SPEC.md` I4) and two samplers on one account would fight over the previous-accepted-position guard of step 4 below. What the hook does there is read the tracker's events, so that every screen goes on reading the same thirteen values from it whichever driver produced them. The samples themselves are unchanged and pass every gate below unchanged; only the side of the app that collected them differs.
...
A teleport is not a way to sample from two places at once: the web app's watch is not started, so the phone's real position is not watched, queued or posted while the mode is on, and the mode ends with an explicit request (Section 9.3) rather than with the next fix. **The sentence is no longer scoped to "this client", because since v1.65 there is no second client to scope it against:** inside the iPhone app the web app posts no sample at all, teleport included.

**What that costs, and what it buys.** The teleport is the web app's own admin fixture and it does not reach the tracker, so honouring it inside the shell meant two posters against one account — the tracker posting the real position while the page posted a faked one, the server's previous-accepted position alternating between the two and refusing some of both on step 4. `ios/SPEC.md` O-I8 recorded that and named two ways out: a bridge message pausing the tracker for the duration, or accepting the interleaving as the cost of a fixture. A third was taken instead, because it needs no bridge message and leaves the tracker the account's sole writer: inside the shell the hook ignores the mode entirely and the Admin screen does not render the teleport panel (`ios/SPEC.md` 8.3, 8.6; Section 8.3 here). **The fixture is not withdrawn** — it is the web app's, and the web app is in Safari on the same phone, signed in to the same account, which is where an admin who wants a twenty-minute mastering test goes. What is withdrawn is the interleaving, which was never anything an admin asked for. The `ios/SPEC.md` copy of O-I8 still reads as open; retiring it there is the `ios-app` branch's own edit, under that document's I7.
...
One known divergence of this path is recorded rather than fixed: it cannot be interleaved with the vector layers at all, so the layer order above does not hold for it (O13).
...
**Step 5 is judged in three places, from one rule.** `isVisitExpired` (`packages/shared/src/visits.ts`) decides, and the maintenance tick (Section 7.9), `GET /api/visits/pending`'s lazy evaluation on read, and `POST /api/samples` all call it rather than restating the arithmetic.

The sample route's share of that is a **sweep over every open pending visit the caller has, once per request, after the batch has been processed** — not only over the visits a sample in the batch was on site for. The distinction is the whole of it: a player who checks in and then walks away produces samples that are on site for nothing, and a visit examined only through the on-site branch is therefore never examined at all. That is precisely the case step 5 exists for, and it went unnoticed for as long as the app stayed open.

Three properties of that sweep are decisions rather than details:

- **It is one pass after the batch, not one per sample.** The caller's pending visits are already in memory — the route loads all of them, keyed by bar, before the sample loop — so the sweep costs no query; per sample it would repeat that pass up to `SAMPLE_MAX_BATCH` times for an answer that cannot change within a request.
- **It runs on the server's clock**, the same one the route stamps `last_sample_at` from and the same one its in-loop expiry test already used. Both sides of the comparison then come from one source. A client timestamp is the wrong basis twice over: Section 7.2 treats it as adversarial input for the teleport guard, and a batch whose samples were all rejected — by the accuracy or speed gates — offers no accepted timestamp to use at all.
- **It runs even when the batch accepted nothing.** It reads no sample, so there is nothing for a rejected batch to deprive it of, and a player standing still with a poor fix is exactly the player whose visit is quietly running out.

The in-loop test stays where it is and is not made redundant by the sweep: it is what stops a late on-site sample extending a visit that was already stale when the batch arrived, which the sweep could no longer see afterwards, because `last_sample_at` would by then be the current second.

**An expiry is reported back to the client, and this reverses an earlier decision.** `POST /api/samples` answers with `visitUpdates`, and a visit the request expired is one of the entries in it (Section 9.6), carrying `status: 'expired'`; the banner drops that row on seeing it. The rule it replaces was that an expiry was written to the database but deliberately never reported, by analogy with `GET /api/visits/pending`'s own lazily-expired rows.

The analogy was wrong, and what separates the two endpoints is what each response *is*. The pending endpoint answers with the pending list itself, so a visit it expires is already absent from its answer and the client learns of it by that absence — an expired entry there would contradict the field's own name and the client would have to strip it out again. The sample response is a list of *changes*, in which absence means "nothing happened to this visit". Keeping an expiry back therefore told the client that nothing had happened to a visit the server had just ended, which is the banner holding a visit the server does not agree is pending — the one thing this section says it must never be able to do. `GET /api/visits/pending` is unchanged and reports nothing new.
...
There is deliberately **no periodic refetch** beside it, and there is now no gap left for one to fill. While the screen is visible the figures in the banner move only when a sample is accepted, and `POST /api/samples` returns both the visits it touched and the visits it expired. A visit running out under a screen that stays continuously visible for the whole of `VISIT_EXPIRY_MS` — the one case a timer would once have caught that this did not — is carried by the next sample post instead, at no request a client was not already making. A timer would cost every open client a repeating authenticated request against the Pi, forever, for a transition that happens at most once per visit.

What that leaves is a screen posting no samples at all — permission refused, or no fix — which learns nothing until it is hidden and shown again. That is the case `visibilitychange` covers, and a client with no position of its own has nothing to measure the banner against in the first place.
...
- **Bars discovered** = count of the player's `bar_discoveries` rows, city-wide.

Every one of these is per player and about the player asking. `GET /api/progress` answers the city-wide area, both city-wide bar counts and the per-district area (Section 9.6); the per-district mastered count is defined here and not yet returned by any route.

**The two bar counts a route returns are scoped to `bars.status = 'active'`, and this is where that differs from the leaderboard.** Section 5.7 hides a bar from play without revoking what was earned at it, so the badge and leaderboard queries deliberately ignore `status` — a hidden bar still counts towards a rank. A progress figure is read beside "bars discovered" and beside a map that no longer draws that bar, so both counts here are drawn from the same set of bars the player can actually see. Nothing is taken away by it: `GET /api/profile/:handle`'s `barsMastered` is the all-time count and is unfiltered.

### 7.7 Badges

Two kinds, three periods, fixed thresholds (Section 7.1). Thresholds are constants in code — not admin-editable in v1 — and since v1.54 they are constants in the **server-only** half of Section 7.1, `packages/shared/src/server-config.ts`. The two badge kinds are client-safe and stay in `config.ts`, because the browser draws a shelf of every badge that exists (placeholders, below) and cannot do that without the catalogue; the six numbers behind them are not, and have exactly one reader in the whole repository, `packages/api/src/badges.ts`.
...
The threshold is **never shown to users, no endpoint returns it, and it is not in the code the browser downloads**. That third clause was added in v1.54 and is the one this section used to get wrong. The first two held literally and were worthless in practice: `CONFIG` is one object literal, `packages/web` imports it as a value, so the whole literal was bundled and the six floors read out of devtools in seconds. A promise about what is rendered is not a promise about what is shipped, and this section now makes both. What enforces the third clause is the module split of Section 7.1 plus `packages/web/src/bundle.test.ts`, which builds the production bundle and fails on a badge period bound to a number anywhere in it — the leak was found in a build, so it is a build that has to prove it gone.

Neither is any rank or standing shown: not "you are 2nd", not "0.3% to go". The profile shows a player their own value for the running period and nothing else, so the only thing a player can read off the game is what they themselves did. Since v1.31 the profile does show something about badges *nobody* has won yet — placeholders, below — and that is a statement about which badges exist rather than about where the player stands; the paragraph that introduces them bounds exactly what they may carry, and none of it moves.
...
Because the tick is cheap and idempotent, a missed tick after a restart is self-healing. Pending-visit status is additionally evaluated lazily on read, so `GET /api/visits/pending` never returns a visit that should already have expired even if the tick has not run, and `POST /api/samples` sweeps the caller's own pending visits on every request (Section 7.5). All three call the same `isVisitExpired`.
...
| Settings | Anonymous toggle, push permission, change password, how-it-works, privacy, delete account, logout — and, **in the iPhone app only**, a "Background tracking" row that shows the consent state of Section 5.3 and opens the app's own consent screen (`ios/SPEC.md` 8.6). That row does not render in a browser, where it would describe a feature the browser does not have; it shows the state and never writes it, because the one client that writes settings is this one and the one path into that column is the app asking it to (`ios/SPEC.md` 5.4, 8.2) |
| Privacy | Static page at `/privacy`, see 10.3 |
| Admin (admins only) | Bar management, community bar moderation, user list, and Section 9.3's teleport with its own map picker — the teleport panel alone, and none of the rest, is absent inside the iPhone app (Section 7.2, `ios/SPEC.md` 8.3) |
...
- **The three figures are all-or-nothing, and they are silent when they fail.** Half a row — a bar count beside a blank where the percentage should be — reads as a broken screen where none of them reads as a screen that simply does not mention them. Since v1.50 that is a property of the shape rather than a rule to keep: all three come off one response, so there is no half to render. Their row is reserved before they exist, so numbers arriving a moment after the first paint cannot lift the action above them out from under a thumb already reaching for it.
...
All three figures come from one request, `GET /api/progress`: the same `city.percent` the city overview renders, so the two screens cannot disagree about how much of Karlsruhe has been walked, and beside it Section 7.6's two bar counts, scoped by the server to this caller, this city and bars that are still active — the same set `GET /api/bars` returns and the map draws its markers from, so a mastered count here and the emptied glasses out there are one fact and not two.

**It fetched the bar list for those two integers until v1.50, and that is what the route now exists to prevent.** `GET /api/bars` ships every discovered bar — name, address, coordinates, district, timestamps — and this screen read a length and a filtered length off it, on the screen every authenticated entry path lands on, growing with exactly the players who play most. The counts moved to `GET /api/progress` (9.2) as two aggregates over one statement, which is the half of the fix that matters: a count the server computed by loading the same rows would have moved the cost rather than removed it. The map still fetches the list, because it draws every bar in it. `GET /api/profile/:handle` remains the wrong source for a different reason: it carries no discovered count, it ships every badge and badge-progress entry this screen does not draw, and it needs the signed-in user's handle to ask at all.
...
- **Connection:** online / offline / syncing, based on `navigator.onLine` plus how far behind this device is on sending. Offline is `!navigator.onLine` and outranks everything else — a device with no connection is not behind on sending, it is unable to send.
  ...
  **That count is one function, `computeBehindDepth` in `packages/shared/src/tracking.ts`** — the samples queued when the attempt began, less the ones it actually sent: the whole batch on a success, and nothing at all on a failure, because a failed post leaves its batch at the front of the queue for the next try. It lives in the shared package for the reason `isVisitExpired` (Section 7.5) and the bar-name comparator (Section 9.3) do: the iPhone app applies this same rule on the phone (`ios/SPEC.md` 7.4, 8.3), and two copies of it are two chances to give one icon two meanings.
...
Per the `RATE_LIMITS` block in Section 7.1, enforced with an in-memory token bucket. Exceeding returns 429 with a `Retry-After` header and Section 9.6's `rate_limited` code. Buckets reset on restart — acceptable at this scale, and stated so it is not mistaken for a durability bug. Each is keyed on what its `by` field there names:

| Limit | Routes (under `/api/`) | What it bounds |
| --- | --- | --- |
| `authGlobal` 60/min | `auth/login`, `auth/reset/question`, `auth/reset` | argon2 work reachable without an account |
| `register` 30/hour | `auth/register` | account spam; the number to raise if the app gets popular |
| `loginByUser` 10 per 5 min | `auth/login` | password guessing against one account, however that account is spelled |
| `resetByUser` 5/hour | `auth/reset/question`, `auth/reset` | security-answer guessing against one account, however that account is spelled |
| `samples` 30/min | `samples` | sample flooding |
| `suggest` 10/day | `bars/suggest` | submission spam (Section 11.3) |

**Both `by: 'username'` limits key on the submitted username trimmed and lowercased**, which is the same account `users.username`'s `COLLATE NOCASE` matches, so `ADMIN` and `aDmIn` do not buy second and third buckets against the account `admin` already has one for.

**No limit is keyed on an IP address, and `by: 'ip'` does not exist.** The `by` union is `'global' | 'user' | 'username'`, so keying on an address would not compile. It was measurably wrong here: behind Cloudflare Tunnel every request arrives from a proxy, so the old `auth` limit's ten attempts a minute were ten for the whole site, and a phone on cellular locked out a laptop on another LAN. The right hop count depends on how many proxies the operator runs and stays silent when wrong; `CF-Connecting-IP` would only trade that topology coupling for a vendor one (Section 13.4).

**Why the global caps are not deleted with it.** Login verifies argon2id against `DUMMY_PASSWORD_HASH` even for an unknown username, on purpose (Section 9.5), so every attempt costs tens of milliseconds of CPU, and a per-username limit does nothing against a caller sending a thousand *different* usernames. `authGlobal` is the only thing here that bounds that: 60/min is about 10 % of one core, set high on purpose, because the old 10/min was low enough for one honest person retyping a password to spend it for everybody. Registration admits no other key — every registration is a new username — and does not also carry `authGlobal`, which is looser and so could never be the limiter to answer there.

**Two costs, accepted rather than discovered.** A global bucket is globally exhaustible: an attacker can hold `authGlobal` or `register` at zero as long as they keep paying. And a named account can be held at its `loginByUser` floor, which refills continuously and so recovers one attempt every 30 seconds — still far smaller than the site-wide lockout it replaces, since that owner is locked out of nothing else and a human's four mistypes go unnoticed.

**Two absences, both deliberate.** The admin surface carries no limit: `RATE_LIMITS` names none for it, every admin route sits behind `requireAdmin`, and the admin account is the trust boundary these limits protect. `POST /api/auth/change-password` carries none either, though it runs argon2 both ways: behind `requireAuth`, abusing it costs a valid session and the account can be dealt with as an account — and adding it to `authGlobal` would let unauthenticated traffic exhaust the bucket legitimate password changes need.
...
| `GET /api/auth/reset/question` | Always 200 with a question. For an unknown username, return a deterministic decoy derived from an HMAC of the username under the server secret, so the response is stable across attempts and indistinguishable from a real one. Rate-limited per username, and under the global auth ceiling (Section 9.4). |
...
| `POST /api/admin/teleport` | Exactly `POST /api/samples`'s body, deliberately — the admin screen renders what happened with the fields the map already understands. It therefore carries `rejected` too, and **all five counts are always zero here**, not only `tooFast`: the synthetic sample's accuracy is `TELEPORT_FIX`'s `0`, its `timestamp` is the same instant the clock gates measure it against, a point outside the bounding box is refused with a 422 before the batch runs at all (Section 9.3), and the speed guard is off. Each of the five is zero for its own reason, so a non-zero count on this route means one of those four things changed rather than that a teleport was refused |
...
**What the client checks at runtime, and what it trusts.** `request<T>` in
`packages/web/src/api/client.ts` ends by returning `body as T` — a cast, not a check — and that is
still true of most of the table above. Four responses opt out of it by passing a hand-written
narrowing predicate from `packages/web/src/api/response-guards.ts` as `request`'s optional third
argument: `GET /api/progress`, `POST /api/samples`, `GET /api/visits/pending` and `POST /api/visits`.
**The rule that selects them: validate where a wrong shape produces a silently _wrong answer_ rather
than a visible failure.** Every field checked is one a screen renders or computes with — a
percentage that reaches `.toFixed(1)`, a duration that reaches the banner's `formatDuration`, a
`status` a branch reads — because a field that moved arrives as `undefined`, `undefined` through
arithmetic is `NaN`, and `NaN` renders as the string "NaN" beside figures that are correct. The
realistic cause is not a hostile server but our own: the service worker (Section 4.1) keeps an
installed shell running against an API that has moved on.

Numbers are checked with `Number.isFinite` and never `typeof x === 'number'`, which accepts `NaN`
and also the `Infinity` a legal JSON literal like `1e999` parses to; `null`, `undefined` and absent
all fail identically, since none of the checked fields is nullable. Fields the client does not read
are deliberately **not** checked even where the table above declares them — `VisitSummary`'s
`startedAt`, `lastSampleAt` and `onsiteSamples`, `GET /api/progress`'s cell counts and district ids
— and neither is anything whose bad shape already fails visibly. So the bar shapes, the boundaries,
`GET /api/city`, `GET /api/leaderboard`, `GET /api/profile/:handle` and the admin routes are all
still cast, `GET /api/fog`'s `X-Fog-Progress` header is cast because no client screen reads
`FogProgress` at all, and `POST /api/visits/:id/cancel` is cast because every caller discards the
`VisitSummary` it answers with. A predicate that rejects throws the ordinary `ApiError` with the
client's own code `invalid_response` and the response's real status, so every screen renders it
through `errorMessage` like any other failure; on `POST /api/samples` the batch stays queued and
retries, which is what lets an updated shell fix it. These are narrowing checks on a handful of
fields and deliberately not a second hand-written copy of the server's zod schemas across the
package boundary. Open Item O18.
...
**Duplicate venues are collapsed on import.** OSM maps the same physical venue twice more often than it should: two nodes surveyed years apart by two mappers, or a node for the venue and a way for the building it occupies. Karlsruhe's seed contained one pair of the first kind. Two rows means two bar ids, and every rule keyed on a bar id then treats them as two bars — including the partial unique index of Section 5.7, which stops a second pending visit at the same id and cannot stop one at the twin. A player standing in front of one building sees two markers an arm's length apart carrying the same name and can check into both.

The rule for deciding that two records are one venue is **Section 11.3's duplicate guard, shared in every part but its radius**: one implementation, the same normalisation, the same `SUGGEST_NAME_SIMILARITY`, and a radius supplied by the caller. The similarity gate is what discriminates, and it is common to both. The radius is not, and the two are separate constants so that neither can be changed by changing the other: the import uses `IMPORT_DUPLICATE_RADIUS_M` (15 m) and the submission form `SUGGEST_DUPLICATE_RADIUS_M` (25 m). The import's is the **tighter** of the two, deliberately. A submission that trips the guard raises a warning on a form a person is filling in and can be overridden by them; a collapse at import removes an OSM record from the seed with nobody looking, so it is the one that has to be conservative.
...
**The 15 m was measured, and it is bounded from both sides.** Across the whole committed Karlsruhe seed there are exactly two pairs that clear `SUGGEST_NAME_SIMILARITY` within 60 m of each other, and only one of them is a duplicate.

- **Fettschmelze**, `node/5075975976` and `node/5408199821`, **6.15 m** apart, carrying the identical address. One venue entered twice. This is what the radius is for.
- **Traube**, `node/1447845917` and `way/54732961`, **25.34 m** apart, neither carrying an address. This is a restaurant and a beer garden on opposite sides of the street — **two real venues**, and both must survive. It reads like a double mapping and it is not one, which is why it is written down here: the shape of the OSM data invites exactly the wrong conclusion, and a radius widened to swallow it would delete a bar a player can walk into. That is a worse failure than the duplicate marker the collapse exists to remove, so the radius is bounded above by this pair and not only below by the one above it.

Any value strictly between those two distances behaves identically on this seed; 15 m is the roundest of them, 2.4× above Fettschmelze and 1.7× below Traube, and it is not derived to any finer precision than that. The similarity gate is what does the rest of the work: every other pair within 60 m has a plainly different name, and the closest any of them comes to the 0.85 threshold is 0.60 ("Old School" against "The Old Chapel Pub"). Re-measure before carrying this number to a second city; it is a fact about how this city is mapped, not a universal one.
...
**A committed seed can be re-collapsed without an import.** `scripts/collapse-seed-duplicates.ts --city=<slug>` (same build requirement) reads `data/seed/<slug>/bars.json`, applies exactly the rule above to it and writes it back, byte-identically formatted, naming every merged pair on stdout. It exists because the seed and the rule can fall out of step for reasons that have nothing to do with OSM: the seed predates the rule (which is how it came to be written — see O19), a constant moves under it, or a curated row is added beside one that was already there. A re-import would fix those too and would be the wrong tool — it needs Overpass, and it drags months of unrelated OSM churn along with the one record actually in question. It is a separate script and not a flag on `import-osm-bars.ts` because that script's whole spine — query, payload validation, tag reading, centroid reduction, grid projection — applies to an Overpass response and not to a file that is already `Bar[]`.
...
| `scripts/collapse-seed-duplicates.ts` | The same `bars.json`, re-collapsed in place under Section 11.1's rule | No — offline, reads and rewrites a committed file |
...
Each phase is independently testable and ends in a deployable state. Do not begin a phase before the previous one's Definition of Done fully passes: every item must be `[x]` — proven in this repository, by a named test or by a named file — or `[~]`, partly proven, with the part that is not proven named. The one exception is an item only the owner's environment can settle: a phone, a GPU, a Lighthouse run, the Pi, a setting on GitHub or Cloudflare that nothing here can see. Those stay `[ ]` with what they need named beside them, so they read as open rather than as unexamined, and they do not gate a later phase, because nothing buildable here would ever move them. An unticked box with no annotation is one nobody has checked, and it does gate.
...
**Definition of Done**
- [~] `docker compose up -d --build` succeeds on the Pi from a clean clone — the image the Pi builds is the **root** `Dockerfile`, through the platform's own compose file and its `deploy.sh` (Section 4.3), and that build and run is recorded in `HANDOVER.md` Section 2 item 4 — observed on the Pi's arm64 by the owner on 2026-08-19, so second-hand here. This repository's own `docker-compose.yml` (api + caddy) has never been brought up anywhere; what is checked here is only that it agrees with `caddy/Caddyfile` — `packages/api/src/compose-caddy-env.test.ts` — and that the root `Dockerfile` carries what the boot reads — `packages/api/src/docker-image.test.ts`
- [x] `https://tipsytrails.ahultsch.com/api/health` returns `{"status":"ok"}` over the public internet — recorded in `HANDOVER.md` Section 2 item 5 and in Section 4.3 above, observed against the live site on 2026-08-19 with `curl`; that record is second-hand and is not something this repository can prove. What this repository proves is the route's own answer: `packages/api/src/app.test.ts`, describe `GET /api/health`: `returns 200 with status ok`
- [~] The SPA shell loads and renders a placeholder page on a phone — a real browser over the public hostname is recorded in `HANDOVER.md` Section 2 item 5 (`/`, the hashed CSS and JS, `manifest.json`, `sw.js` and every icon all 200, plus a 304 on reload), second-hand as above; the shell is also served and asserted here (`packages/api/src/app.test.ts`, describe `SPA static serving`). **On a phone is unverified** — there is no phone here
- [x] Migrations run idempotently on container restart, tracked in `schema_migrations` — `packages/api/src/db/migrate.test.ts`: `applies the migration once and is a no-op on rerun` (the second run applies nothing and `schema_migrations` still holds exactly the three rows); `packages/api/src/startup.test.ts`: `is idempotent on a second call against the same path`, which is what a container restart does to the same database file
- [~] No secret is present anywhere in the repository, and secret scanning with push protection is enabled — the first half is verified by reading: `.env` is gitignored, `.env.example` carries variable names and shapes with no values (`SESSION_SECRET=`, `ADMIN_USER=`, `ADMIN_PASSWORD=`, `VAPID_*=`), and `git ls-files` tracks no `.env`, no database and no key file. **Secret scanning and push protection are a GitHub repository setting nothing here can read** — only the owner can confirm them
- [x] `LICENSE` (MIT) and `DATA-LICENSE` (ODbL) exist, and the README states which covers what — verified by reading: `LICENSE` is the MIT text, `DATA-LICENSE` is the ODbL 1.0 notice and names the artefacts it covers, and `README.md`'s "Licensing" section states the split (MIT for the code; ODbL for everything under `data/seed/<slug>/` and the `.pmtiles` extract). No test asserts this
- [x] `data/db/` and `data/tiles/` are gitignored; a fresh clone contains no database file — verified by reading: `.gitignore` carries `data/db/*` with `!data/db/.gitkeep`, and `data/tiles/` entire; `git ls-files data/` lists only `data/cities/karlsruhe.json`, `data/db/.gitkeep` and the six files of `data/seed/karlsruhe/` — no `.db`, no `.pmtiles`
- [x] The admin account is seeded from environment variables, never from code, with `must_change_password = 0` (Section 5.3: the platform supplies and rotates this credential, so it must stay valid) — `packages/api/src/db/seed-admin.test.ts`: `seeds a single admin user with is_admin set and must_change_password clear`, `creates no user when ADMIN_PASSWORD is absent, and says so as 'no-credentials'`, `creates no user when ADMIN_USER is absent, and says so as 'no-credentials'`; `packages/api/src/startup.test.ts`: `seeds an admin when ADMIN_USER and ADMIN_PASSWORD are present`, `leaves the user table empty when admin variables are absent`; and for a database seeded before the rule existed, `packages/api/src/db/migrate.test.ts`: `clears must_change_password for an admin that carried it before the migration`
- [ ] Container memory at idle < 150 MB total — needs the Pi, or any Docker host; no container has ever been run in this environment
...
**Definition of Done**
- [~] A user can register, log out, and log back in on a phone — the round trip is proven against the API: `packages/api/src/routes/auth.test.ts`: `succeeds, sets a session cookie, and creates exactly one user row`, `clears the session so the cookie no longer works and removes the sessions row`, `succeeds with correct credentials and sets a cookie`; and the register screen drives it in the browser-shaped tests, `packages/web/src/App.test.tsx`: `does not submit the register form while the 18+ box is unchecked, and does once it is ticked`. **On a phone is unverified** — jsdom is not a phone and there is no device here
- [x] Registration is rejected without the 18+ confirmation — `packages/api/src/routes/auth.test.ts`: `rejects registration when ageConfirmed is missing and creates no user`, `rejects registration when ageConfirmed is false and creates no user`; client-side, `packages/web/src/App.test.tsx`: `does not submit the register form while the 18+ box is unchecked, and does once it is ticked`
- [x] Password reset works end to end via the security question, and invalidates existing sessions — `packages/api/src/routes/auth.test.ts`, describe `POST /api/auth/reset`: `resets the password and invalidates every existing session for that user`, `matches the answer case-insensitively and ignoring surrounding whitespace`, `returns byte-identical responses for a wrong answer and an unknown username`
- [x] An unknown username returns a stable decoy security question, indistinguishable from a real one — `packages/api/src/routes/auth.test.ts`, describe `GET /api/auth/reset/question`: `returns 200 with the same body shape and a non-empty question for an unknown username`, `returns a stable decoy for the same unknown username across requests`, `returns different decoys for different unknown usernames`, `does not depend on whether other users exist`, and the four normalisation cases that keep a real and an unknown username answering the same way across casing and padding
- [x] The seeded admin signs in **without** a forced password change (`must_change_password = 0`, Section 5.3), and an account that does carry the flag is confined to `/api/auth/me`, `/api/auth/change-password` and `/api/auth/logout` until it clears it — the flag: `packages/api/src/db/seed-admin.test.ts`: `seeds a single admin user with is_admin set and must_change_password clear`, `leaves must_change_password at 0 once the admin has changed it`, and describe `seedAdminRotatingPassword against a live session`, which signs the seeded admin in over HTTP and reaches an authenticated route with the cookie it gets back; `packages/api/src/startup.test.ts`: `seeds an admin when ADMIN_USER and ADMIN_PASSWORD are present`. The confinement: `packages/api/src/auth/password-gate.test.ts`, describe `must_change_password gate`: `returns 403 with code password_change_required on a normal authenticated endpoint`, `still allows GET /api/auth/me while the flag is set`, `still allows POST /api/auth/change-password while the flag is set`, `still allows POST /api/auth/logout while the flag is set`, `lets the previously blocked endpoint through after the password is changed`
- [x] Passwords and security answers are argon2id hashes in the database — verified by inspection — `packages/api/src/routes/auth.test.ts`: `stores argon2id hashes for the password and security answer, and the plain security question` (reads the columns back out of SQLite and checks the `$argon2id$` prefix); `packages/api/src/db/seed-admin.test.ts`: `stores an argon2id password hash`
- [x] Session cookie is `httpOnly`, `Secure`, `SameSite=Lax` — `packages/api/src/auth/cookie.test.ts`, describe `setSessionCookie`: `sets HttpOnly, SameSite=Lax, Path=/ and Secure when PUBLIC_ORIGIN is https`, and `does not set Secure when PUBLIC_ORIGIN is http`, which is the local-development case; the deployment's `PUBLIC_ORIGIN` is `https://tipsytrails.ahultsch.com` (Section 4.3)
- [x] Rate limits on auth endpoints return 429 when exceeded, **and no bucket anywhere is keyed on the client's address**: exhausting login for one username leaves every other username able to log in, whatever `X-Forwarded-For` the attempts carry (Section 9.4) — `packages/api/src/routes/auth.test.ts`, describe `rate limiting on auth endpoints`: `returns 429 after exceeding the per-username login limit`, `keeps one login bucket per username however many addresses the attempts come from`, `leaves a second username able to log in once the first one is exhausted`, `answers 429 on login once the global ceiling the reset routes share is spent`; and the limiter itself, `packages/api/src/http/rate-limit.test.ts`: `puts every caller in one bucket whatever address the request appears to come from`, `keeps one bucket per username however many addresses the requests claim to come from`
- [x] Account deletion requires the password and removes every row belonging to the user — `packages/api/src/routes/account.test.ts`, describe `DELETE /api/account`: `rejects a wrong password without deleting, returning the same generic failure as login`, `hard-deletes the user and cascades every referencing table, while shared catalogue data survives`
...
**Definition of Done**
- [ ] Karlsruhe renders in the ink style on a mid-range Android phone — needs a phone and a real GPU; jsdom draws no map and there is neither here. What is proven is the style document MapLibre would be given: `packages/web/src/map/ink-style.test.ts`
- [ ] Pan and zoom hold ≥ 50 fps — needs a device to measure on
- [x] The tile route answers `206 Partial Content` to a ranged request (Section 4.1) — `packages/api/src/routes/tiles.test.ts`, describe `GET /tiles/:filename with a present extract`: `returns 206 with the exact 100-byte slice for Range: bytes=0-99`, `returns the matching slice for a second, different range`
- [ ] Tiles are served with range requests and cached at the Cloudflare edge (verified via `cf-cache-status: HIT`), with the Cache Rule from Section 4.1 in place — needs the owner's Cloudflare dashboard and a `curl -I` against the live site. The Cache Rule is not in place (`HANDOVER.md` Section 2 item 2, open) and the extract is not on the Pi's volume either (item 1), so there is nothing at the edge to hit yet. The origin half is proven here: the two range tests above, and `carries the cacheable Cache-Control header, not no-store` in the same file
- [x] City overview → district overview → map navigation works, neighbouring municipalities are greyed and inert — `packages/web/src/App.test.tsx`: `renders the city outline and the greyed-out, non-interactive neighbour shapes on /city`, `renders all 27 districts as a list with a name and a percentage each`, `navigates to the map when a district in the list is tapped`, and describe `picking a district on the schematic map`: `points the panel's link at the same URL as that district's list item`, `carries the district's bounding box, and its centre, in the link`
- [x] OSM attribution is visible and links correctly — `packages/web/src/App.test.tsx`: `renders the map screen with an OSM attribution link to the copyright page`, `still shows the OSM attribution when the tile availability check fails outright`
- [x] App shell < 150 KB gzipped, excluding the map chunk; the map chunk (MapLibre + PMTiles, ~250 KB gzipped) is code-split and loaded only on map routes. A single bundle under 200 KB is not achievable with MapLibre and must not be attempted by trimming features. — asserted since v1.58 by `packages/web/src/bundle.test.ts`: `keeps the eagerly loaded app shell inside its gzipped budget` and `reaches the MapLibre chunk only through a dynamic import`, both against the production build that file already runs on every `pnpm test`. The first derives the eager set rather than naming chunks (which carry content hashes): start at the entry chunk, follow static `imports` transitively, gzip those chunks and the stylesheets they pull in, and compare the total against 150 000 bytes, reporting the measured figure and the per-chunk breakdown when it fails. That total is **82.0 kB gzipped** at v1.58 — 76.9 kB of JS across four chunks plus 5.0 kB of CSS — against the 82.7 kB read by hand off Vite's own report on 2026-09-04, the same build measured by `gzipSync` rather than by the reporter. The second holds the split, which the size alone does not: no eagerly reachable chunk contains MapLibre, and the chunk that does — 223.7 kB gzipped, plus 9.2 kB of its own stylesheet and the 7.5 kB map-screen chunk that imports it, which is the 239 kB recorded here before — is reached only across a `dynamicImports` edge, from the `lazy(() => import(...))` route splits in `packages/web/src/App.tsx`. No upper bound is asserted on the map chunk itself: "~250 KB" describes what MapLibre and PMTiles weigh and is not a limit anyone chose
...
**Definition of Done**
- [x] Walking reveals a ~100 m radius; the mask survives logout and reload — `packages/api/src/routes/fog.test.ts`, describe `POST /api/samples`: `reveals roughly a 100 m radius (~13 cells at 50 m, SPEC.md Section 7.3)`; and describe `GET /api/fog`: `the mask survives across a fresh app instance on the same database`, which is the reload. Logout cannot touch it and no test needs to prove that separately: the mask is a `fog_state` row keyed by user and city (Section 5.5), and `POST /api/auth/logout` deletes the session row and nothing else (`packages/api/src/routes/auth.ts`, and `packages/api/src/routes/auth.test.ts`: `clears the session so the cookie no longer works and removes the sessions row`)
- [x] Travelling above 30 km/h reveals nothing — `packages/api/src/routes/fog.test.ts`: `reveals nothing for a sample above FOG_MAX_SPEED_KMH, and says that is why`, `refuses just above FOG_MAX_SPEED_KMH and reveals just below it`, `reports a speed it derived itself for a sample the device gave no speed for`, `lets the last accepted sample of a mixed batch decide the speed verdict` (`CONFIG.FOG_MAX_SPEED_KMH` is 30)
- [x] Samples with accuracy > 200 m are discarded; so are stale and future-dated samples — accuracy: `packages/api/src/routes/fog.test.ts`: `discards a sample with accuracy worse than FOG_MAX_ACCURACY_M entirely` (`CONFIG.FOG_MAX_ACCURACY_M` is 200). Stale: `packages/api/src/routes/visit-samples.test.ts`: `a sample rejected as stale by Section 7.2 validation does not touch the visit`, which posts a sample older than `CONFIG.SAMPLE_MAX_AGE_MS` and gets `newCells: 0` back. Future-dated, tested at v1.58: `packages/api/src/routes/fog.test.ts`, describe `the clock-skew guard`: `discards a sample dated further ahead than SAMPLE_MAX_CLOCK_SKEW_MS` (which also reads `GET /api/fog` back to show nothing was written), `does not let a discarded future-dated sample poison the teleport guard` (a rejected sample must not become the previous accepted position the next sample's speed is measured against; the control half of the same test posts the identical fixture with a present timestamp and does see the next sample refused), `accepts a sample dated inside SAMPLE_MAX_CLOCK_SKEW_MS`, and — on a clock frozen with `vi.useFakeTimers`, the only way the boundary is reachable from outside — `accepts a skew of exactly SAMPLE_MAX_CLOCK_SKEW_MS and refuses one millisecond more`, the comparison in `packages/api/src/routes/fog.ts` being `skewMs > CONFIG.SAMPLE_MAX_CLOCK_SKEW_MS`, so exactly the tolerance is accepted. That a rejected future-dated sample also touches no visit is `packages/api/src/routes/visit-samples.test.ts`: `a sample rejected as future-dated by Section 7.2 validation does not touch the visit`
- [ ] Fog edges are soft and irregular — no visible grid squares, no hard circle — this is about how the fog looks, and nothing here has ever drawn it: there is no GPU in this environment and the GLSL has never been compiled (`HANDOVER.md` Section 2 item 14). It needs a real render and human eyes. What is asserted is the shader's construction, which is not the same claim: `packages/web/src/map/fog/webgl-fog-layer.test.ts`: `bakes the edge blur radius and alpha band into the shader from CONFIG`, `keeps the visible fog transition inside a single grid cell`, `layers the density noise over a few octaves at ratios that cannot align`, `keeps the finest density feature several grid cells across`
- [x] District and city percentages are correct against a hand-computed reference — `packages/api/src/routes/fog.test.ts`, describe `GET /api/progress`: `percentages match a hand-computed reference from grid-meta.json`
- [x] `fog_daily_progress` sums equal `fog_state.revealed_cells` for a test user — `packages/api/src/routes/fog.test.ts`: `fog_daily_progress sums to fog_state.revealed_cells for the user`
- [ ] The fog layer holds ≥ 50 fps during continuous panning — needs a device with a GPU to measure on; see the shader item above
- [~] The 2D canvas fallback renders correctly with WebGL2 disabled — that the fallback is what gets chosen when there is no WebGL2 is proven: `packages/web/src/App.test.tsx`: `fetches the fog mask and grid on mount and renders the 2D canvas fallback (jsdom has no WebGL2)`. What it draws is proven as a sequence of canvas operations against a fake 2D context: `packages/web/src/map/fog/canvas-fallback.test.ts`: `punches exactly one destination-out hole per revealed cell`, `punches holes one cell across, not the one and a half from the grid boundary`, `keeps the punched holes a full cell wide on a rotated map`, `keeps the padding ring outside the grid fogged even when every cell is revealed`, `redraws on moveend and not otherwise`. **No pixel is ever rasterised here** — "renders correctly" in the sense of looking right needs a browser
- [x] The GPS/connection indicator reflects real state, including the foreground-only note — the state machine: `packages/web/src/tracking/status.test.ts`, describe `computeGpsStatus` (the good/fair/poor boundaries, and `goes poor once GPS_STALE_MS has passed since the last fix, even with good accuracy`) and describe `computeConnectionStatus` (offline, online, syncing). Wired to the screen: `packages/web/src/App.test.tsx`, describe `position sampling and the status indicator`: `maps GPS accuracy to good, fair and poor at the configured boundaries`, `goes to GPS poor after GPS_STALE_MS with no further fix`, `reports syncing for samples a full batch left behind, and counts every unsent one`, `opens the tracking status explanation when the indicator is tapped`, and `names the button from its contents: what it opens, then the three states`, which is the one that asserts the foreground note reads `Foreground tracking: paused`
- [x] No raw position is written to the database — verified by inspection — `packages/api/src/routes/fog.test.ts`: `never stores anything resembling the submitted raw coordinates`, which posts two samples and then sweeps every column of every row of every table for a number within 1e-6 of a submitted coordinate, or a string containing one
...
**Definition of Done**
- [x] `data/seed/karlsruhe/bars.json` is committed and imports cleanly — the file is tracked (`git ls-files data/seed/karlsruhe/`), and the import runs against that committed file rather than a fixture: `packages/api/src/db/seed-bars.test.ts`, describe `seedBars against the committed data/seed/karlsruhe/bars.json`: `imports every row with a resolved district_id, cell_index and city_id`; re-import is covered by `is idempotent: a second boot changes nothing` and `updates an existing bar in place by osm_id, keeping its id stable`
- [x] Approaching within 100 m discovers a bar; it persists after reload — `packages/api/src/routes/bars.test.ts`, describe `bar discovery via POST /api/samples`: `creates a discovery and reports it in newBars for a sample within BAR_DISCOVERY_RADIUS_M` (`CONFIG.BAR_DISCOVERY_RADIUS_M` is 100), `discovers nothing for a sample just outside BAR_DISCOVERY_RADIUS_M`, `reports no bar again once already discovered`. The persistence is the `bar_discoveries` row that first test asserts, read back by a later, separate `GET /api/bars` — `returns only bars discovered by the requesting user, and a second user sees their own` — which is what a reload re-issues
- [x] `/api/bars` returns only bars discovered by the requesting user — `packages/api/src/routes/bars.test.ts`, describe `GET /api/bars`: `returns only bars discovered by the requesting user, and a second user sees their own`
- [x] `/api/bars/:id` returns an identical 404 for an undiscovered bar and a nonexistent one — `packages/api/src/routes/bars.test.ts`, describe `GET /api/bars/:id`: `returns a byte-identical 404 for an undiscovered bar and for a nonexistent id`
- [~] No endpoint leaks the existence or position of undiscovered bars, including counts — proven of every player-facing surface that touches bars: the two tests above; `packages/api/src/routes/fog.test.ts`, describe `the two bar counts`, where `GET /api/progress` answers with the caller's own discovered and mastered counts and no city total (`are zero for a player who has discovered nothing`, `counts neither figure for another player’s discoveries and mastery`); and `packages/web/src/App.test.tsx`: `never renders a count of undiscovered bars anywhere on the map screen`. **The universal claim is not proven**: no test enumerates the route table to establish the negative for endpoints yet to be written, and `/api/admin/*` deliberately returns every bar, undiscovered ones included, behind `requireAdmin` (Section 9.3)
...
- [x] Step C — the server's two additions: `rejected` on `POST /api/samples` and the teleport, and the consent column behind the widened `PATCH /api/settings` — Definition of Done in `ios/SPEC.md` Section 12, Step C. All four of its items, over two releases: `rejected` on both routes with a test per gate, the mixed batch and the two teleport tests, the web response guard still passing on the new shape (v1.60); the migration, the column on every route that answers with a user, and the three asserted absences (v1.61); the partial body with a test per row of `ios/SPEC.md` 9.2's table, the older `PATCH /api/settings` tests passing unedited (v1.61); and the cascade, which `001_init.sql` already gave and a test now says so (v1.61). Step C is `packages/api` and `packages/shared` work carried out here rather than on `ios-app`, per that document's I7. It also said Step E was unblocked on this side, which was incomplete: the harness has to open and migrate a database of its own, and `packages/api` exported neither `openDatabase` nor `runMigrations` until v1.65's row 14
- [x] Step D — the web app's shell detection and bridge, and the tracking hook's second driver behind them — Definition of Done in `ios/SPEC.md` Section 12, Step D. All six of its items, over three releases: the shell driver's three spies that must not be called, a test per row of `ios/SPEC.md` 8.3's thirteen-member table, and `SampleTrackingState` asserted against the type so a fourteenth member fails that item (v1.62); the four counters starting at nought and never advanced by the bridge's replayed payload, asserted with `useBarStamps` and `useVisits` mounted against a bridge that already holds one (v1.62); the indicator's four states with their words, one shape across all six renderings and an accessible name that states the state (v1.63); the push offer posting `requestNotifications` under the shell, with `usePushSubscription` reporting `unsupported` there unchanged (v1.63); the Settings row rendering under the shell and not in Safari (v1.63), and `/privacy` carrying the app's section everywhere (v1.62); and `ready`, `signedIn`, `signedOut`, `visitStarted` and `visitEnded` posted at the moments 8.2 fixes, `signedOut` before the logout request asserted by order (v1.62). Step D is `packages/web` work carried out here rather than on `ios-app`, per that document's I7
...
| `data/seed/karlsruhe/city.geojson`, `districts.geojson`, `neighbours.geojson` | Yes | OSM-derived, ODbL — all three written by `scripts/fetch-boundaries.ts` (11.4), and all three named in `DATA-LICENSE` |
| `data/seed/karlsruhe/grid.bin`, `grid-meta.json` | Yes | Reproducible via `scripts/build-grid.ts`, from `districts.geojson`; ODbL with it (13.3) |
| `data/cities/<slug>.json` | Yes | The pipeline's config seam (11.4) — script parameters, not map data |
...
- **Map data and derived artefacts** (everything under `data/seed/<slug>/`, plus `*.pmtiles`): ODbL, inherited from OpenStreetMap. `DATA-LICENSE` names each file and carries the attribution required by Section 10.5.
...
1. **No security through obscurity.** Rate limits, the session model, and the security-question reset flow are all publicly readable. They are specified to hold up under that assumption, and the rate limits in Section 9.4 are load-bearing, not decorative — an open-source app anybody may self-host is also why none of them is keyed on an address.
...
| O19 | **Closed by doing it a different way, and by one of its premises being wrong.** This asked for the committed seed to be re-collapsed, correctly: `data/seed/karlsruhe/bars.json` held the pre-collapse 180 records, and a fresh deployment got two "Fettschmelze" markers an arm's length apart that a player could check into twice. Two things about its stated fix did not hold. Its remedy — "re-run the import (owner's machine — it needs Overpass)" — was never run and is no longer the right one: `collapseDuplicateBars` is a pure function over `Bar[]`, so `scripts/collapse-seed-duplicates.ts` (Section 11.1) applies the rule to the committed file offline, deterministically, with no unrelated OSM churn. And its predicted **178** assumed Traube was one venue mapped twice; it is two venues, so only Fettschmelze collapsed and the file holds **179** (`node/5075975976` survived `node/5408199821`, 6.15 m apart, on the older-id rule). The radius that would have produced 178 would have deleted a real bar — see Section 11.1 and trap 14 below, where that fact is kept for the next person to widen it. Row kept rather than deleted, per this section's rule: a vanished O19 would read as "the import was re-run and produced 178", which is the one thing that did not happen. | Closed in v1.49 |
...
- **v1.65** — Rows 12, 13 and 14 of `ios/SPEC.md` Section 12's list for `main`, which between them complete that list. The brief was GitHub issue #2, the `for-ios` channel `ios/PARENT-CONTRACT.md` Section 0 describes, and it was the authority where it disagrees with this repository's copy of `ios/SPEC.md`: that copy is v0.8 and the rows were written at v0.11 on `ios-app`, which is the one tree I7 does not let `main` edit. **`@tipsytrails/api` exports `openDatabase` and `runMigrations`** (row 14, taken first because `ios/SPEC.md` Step E is blocked on it and starts the moment it merges). `buildApp(env, db)` takes an already-open database, and 13.2's replay harness is what has to open and migrate one — a temporary database, the migrations in `packages/api/migrations`, then `buildApp` — so with neither step exported and `packages/tracker` unable to depend on `better-sqlite3` itself (I7), the harness could not take them at all. v1.60's claim that it was unblocked was therefore incomplete, and Phase 9's Step C entry in Section 12 is corrected rather than left standing. The type goes with the pair, as `SqliteDatabase`: a consumer that cannot import `better-sqlite3` cannot name what `openDatabase` answers with either, and a database it can hold only as an inferred local cannot cross a function boundary in its own code. Nothing else in the API changed — in particular no wrapper that opens and migrates in one call, because `src/startup.ts` already composes the two in the order a server wants and a second composition would be a third place that decides what an opened database is. `packages/api/src/package-entry.test.ts` asserts the pair exactly as it already asserted `buildApp`, through the package name and resolved from `packages/tracker`'s own position in the workspace, and gains two cases it did not have: the three signatures pinned against `SqliteDatabase` by annotation, since the entry could export both functions and a type that had drifted from them while every `typeof` check still passed, and a count of the entry point's runtime exports, since a list of re-exports is meant to stay one and writing the list down is what makes the next addition a decision. The rule that file exists for is untouched: nothing in the entry runs on import, and `src/server.ts` is still deliberately not what the package resolves to. **`requestSettingsUpdate` gets a reply** (row 12). The shell's native Consent screen ticked the box, called the page, and went on to iOS's Always prompt with no way to know whether the account had recorded anything; the outcome reached it from the tracker's next `start` re-reading `GET /api/auth/me`, which is a different question answered much later. So the page's handler returns `true`, which the shell reads through `evaluateJavaScript`'s completion handler, and a ninth Page → shell message carries the outcome when the `PATCH /api/settings` settles: `{ type: 'settingsUpdated', backgroundTracking, ok, reason? }`, exactly once per request, with `backgroundTracking` echoing the request because the shell can have ticked and withdrawn before either answer arrives. A page that returns nothing is a page with no reply coming and the shell keeps its old behaviour, which is why the return type is `true` rather than `boolean` — the no-reply case is the absent property. `reason` is `offline` for a transport failure that never got an HTTP status, `unauthenticated` for a 401 and `server` for every other status including the schema's own 400; it is read off `ApiError`'s status and not its message, and the two cases that reach `server` by falling through — a 2xx body the page cannot read, and the synchronous throw nothing today can produce — are written down, because calling either of them `offline` would tell a player with a working connection to try again later. The message is two union members rather than one with an optional `reason`, which is the same shape on the wire and a stronger one in the page: one member with `ok: boolean` and `reason?:` lets an `ok: true` carry a reason with nothing failing. The console line a failure already wrote stays; it has a different reader. **And the web app posts no sample inside the shell, without exception** (row 13), which decides `ios/SPEC.md` 15's O-I8. The shell driver ignores `TeleportMode` entirely and the Admin screen does not render the teleport panel there — the mirror image of the Settings row of `ios/SPEC.md` 8.6, which renders only under the shell. The third way out of O-I8 is the one taken: not a bridge message pausing the tracker, not accepting the interleaving, but the page simply not posting, which leaves the tracker the account's sole writer and needs nothing from the bridge. Section 7.2's closing sentence loses the "this client" scope v1.62 gave it and Section 8.3's Admin row gains a clause; the fixture itself is not withdrawn, because it is the web app's and the web app is in Safari on the same phone. `SampleTrackingState` still has exactly thirteen members, the browser's teleport is untouched and its suites are unedited, and what the change deletes is a branch rather than adding one: the hook reads the mode as `off` under the shell, so the `queue` and `flush` events write the indicator's counts unconditionally again, and `queueRef` has two writers both of which only the browser reaches — which is the whole proof that nothing is posted. The `ios/SPEC.md` copy of O-I8 and of 8.2's "the shell does not read a reply to this call" are not edited here; both are `ios-app`'s, under I7, which allows `main` the parent pin, `ios/PARENT-CONTRACT.md` Sections 1 and 3, and nothing else in that tree. No Phase 9 step changes status: Step D was ticked at v1.63 and rows 12 and 13 extend the surface it built, while Steps E, F and G are built on `ios-app`. Thirteen tests, three in `packages/api` and ten in `packages/web`; one existing test was rewritten rather than edited — `App.shelltracking.test.tsx`'s standing-teleport case asserted the two-poster behaviour O-I8 recorded, and now asserts the three things the tracker took over from it. Sections 7.2, 8.3, 12, 15; `packages/api/src/index.ts`, `packages/api/src/package-entry.test.ts`, `packages/web/src/shell/messages.ts`, `packages/web/src/shell/bridge.ts`, `packages/web/src/shell/useShellSettingsUpdate.ts`, `packages/web/src/tracking/useSampleTracking.ts`, `packages/web/src/screens/Admin.tsx`, `packages/web/src/App.shell.test.tsx`, `packages/web/src/App.shelltracking.test.tsx`, `packages/web/src/App.community.test.tsx`, `ios/SPEC.md` (the parent pin), `ios/PARENT-CONTRACT.md` (Section 1's D1, D7 and E2, and one merge-record row), `HANDOVER.md`. 1809 tests (shared 248, api 599, web 729, tracker 233).
- **v1.64** — Row 9 of `ios/SPEC.md` Section 12's list for `main`, the last row of that list, **amended before it was built**. The row asks `useSampleTracking` to adopt `computeBehindDepth` from `packages/tracker/src/queue.ts`; it cannot, because `packages/web` imports nothing from `packages/tracker` and must not start — the tracker is the phone's package, and the website depending on the phone inverts the direction `ios/SPEC.md` 8.3 already pays for by mirroring the tracker's event union by hand in `packages/web/src/shell/events.ts`. **So the rule moves to `packages/shared/src/tracking.ts` instead**, which both sides already depend on, and `useSampleTracking.flush()` calls it at both of its call sites — `computeBehindDepth(queuedAtAttempt, batch.length)` on a success and `computeBehindDepth(queuedAtAttempt, 0)` on a failure — in place of the inline subtraction it applied before. **Zero behaviour change, and the evidence is that no existing test was edited**: the two implementations were compared first and already agreed exactly, so this is one arithmetic rule gaining one home rather than a rule being decided. Section 8.6 gains the sentence naming that home, for the reason it names `isVisitExpired` and the bar-name comparator: the same subtraction runs on the phone (`ios/SPEC.md` 7.4), and two copies of it are two chances to give one icon two meanings — a section that already records shipping the wrong rule once. **What this block does not do is delete the tracker's copy.** `packages/tracker/` belongs to `ios-app` under `ios/SPEC.md` I7, and deleting it here would break that branch's build the moment it merges; so two implementations of one rule still exist, and closing that is `ios-app`'s to do when it adopts the shared export. This block halves the problem rather than ending it, which is not what row 9 was written to achieve. `ios/PARENT-CONTRACT.md` Section 1's **D2 moves**: the definition of `behind` is no longer only in `packages/web/src/tracking/status.ts`, so the entry and the fast check both gain the new shared module — without that, a change to the one function both clients read would move nothing on that document's fast check. D2's claim that Step B built a shared test fixture for `behind` is corrected in the same edit; `ios/SPEC.md` v0.5 had already recorded that no such fixture was ever built. Five tests, all in `packages/shared/src/tracking.test.ts`: the two call shapes, the boundary where the batch carried exactly what was queued, an empty queue, and the sign on its own, because an inverted subtraction is not an off-by-one — it is `syncing` on every healthy phone. The two call sites were already covered indirectly through `connectionStatus` and gained nothing new. Sections 8.6, 15; `packages/shared/src/tracking.ts`, `packages/shared/src/index.ts`, `packages/web/src/tracking/useSampleTracking.ts`, `ios/SPEC.md` (the parent pin), `ios/PARENT-CONTRACT.md` (Section 1's D2 and the fast check, and one merge-record row), `HANDOVER.md`. 1796 tests (shared 248, api 596, web 719, tracker 233).
- **v1.63** — Rows 5, 6 and 7 of `ios/SPEC.md` Section 12's list for `main`, which complete that document's Step D. **The tracking indicator's third icon carries four states inside the iPhone shell** (Section 8.6, `ios/SPEC.md` 8.3): on in the background (ok), on while open only (degraded), blocked (bad) and idle (bad). They reach `TrackingIndicator` from a new `packages/web/src/shell/useShellTracking.ts` and **not** from `SampleTrackingState`, which is what keeps that interface at the thirteen members v1.62 pinned — a fourteenth would be a member every screen inherits and every future driver has to produce, for one component's benefit. The derivation is the `tracking` event of `ios/SPEC.md` 7.5 and nothing else: `background` separates the first two states, and where it is false the authorization separates When In Use from a consent the account has not given, because 7.3 keeps `background` false whatever the ladder says until `GET /api/auth/me` reports consent. The degraded state's panel offers `openConsent`; the blocked state's names the cause, and `servicesOff` says "Location Services is off" rather than naming the app's own permission, which `ios/SPEC.md` 6.5 requires because iOS reports a refusal and the device-wide switch identically. **The shapes do not change** — the icon markup takes a level and a name and no state, and a test collects the rendered shape across all four shell states and both browser states and requires one — and each accessible name states the state, extending the existing `GPS signal: poor` pattern rather than inventing a second one. A fifth rendering, `not reported yet`, covers the shell not having answered yet: `idle` means "not signed in" and `blocked` means the platform refused, so saying either would be the indicator asserting something about the phone it has not been told. **The push offer on `screens/HowMasteringWorks.tsx` becomes a `requestNotifications` button under the shell** (`ios/SPEC.md` 8.4). `usePushSubscription` needed no change and got none: `pushSupported()` already requires `PushManager`, which a `WKWebView` does not expose, so the hook reports `unsupported` there on its own and the change is the button's destination. **The Settings screen gains a "Background tracking" row, under the shell only** (`ios/SPEC.md` 8.6), reading `backgroundTrackingConsentedAt` off the account and opening the app's consent screen with `openConsent`. It shows the state and does not write it — the write path is the shell calling `requestSettingsUpdate`, which this page answers with `PATCH /api/settings` (v1.62) — so there is still one writer of that column and no race with iOS's own Always prompt. Section 8.3's Settings row is amended to name it, because that table is the parent's inventory of what a screen holds and it had become incomplete for one platform. Every one of the three is asserted twice, once under an injected shell and once in a browser with a message handler installed and listening: Section 8's rule is that each change is behind one detection and a no-op outside it, and a no-op is a property to assert rather than infer. `requestNotifications` and `openConsent` were built and left uncalled at v1.62 and now have their callers; `openExternal` stays uncalled, and `ios/SPEC.md` 8.5 says why — the shell's own delegates already send every off-origin link to Safari without the page knowing. Twenty-two tests, in `packages/web/src/App.shellscreens.test.tsx`; one existing assertion in `App.shelltracking.test.tsx` moved, because a `tracking` event with `background: false` used to make the icon ok and now correctly makes it degraded. Section 12's Phase 9 Step D is ticked, all six of its Definition-of-Done items now built across v1.62 and v1.63. Sections 8.3, 12, 15; `packages/web/src/shell/useShellTracking.ts`, `packages/web/src/components/TrackingIndicator.tsx`, `packages/web/src/screens/HowMasteringWorks.tsx`, `packages/web/src/screens/Settings.tsx`, `packages/web/src/api/types.ts`, `ios/SPEC.md` (the parent pin), `ios/PARENT-CONTRACT.md` (Section 1's D7 and one merge-record row), `HANDOVER.md`. 1791 tests (shared 243, api 596, web 719, tracker 233).
- **v1.62** — Row 4 of `ios/SPEC.md` Section 12's list for `main`, the second of that document's five Step D substeps and the one it calls "the largest risk in this plan: it puts a seam through the hook every screen reads". **`useSampleTracking` gains a driver, chosen once at mount.** In every browser it is the `watchPosition` driver it has always been; inside the iPhone shell it is a driver that subscribes to the tracker's events through `packages/web/src/shell/bridge.ts` and starts no watch, takes no wake lock, keeps no queue of its own and posts nothing, because the tracker is the only sampler there (`ios/SPEC.md` I4). The seam is one `if` on a mount-time boolean inside the one effect that was already there, with the two arms behind `attachGeolocationDriver` and `attachShellDriver`; everything both drivers need — the teleport, `flush()`, the `online`/`offline` listeners, the cadence interval and the new `applyServerAnswer` — is shared between them rather than written twice, which is what keeps one `computeConnectionStatus` over one `behindDepth` and stops Section 8.6's `syncing` meaning two things on one icon. Three alternatives were weighed in the file: two hooks behind a dispatcher (a hook cannot be called conditionally), a driver object passed in by the caller (`screens/Map.tsx` would then have to know which platform it is on), and moving the browser path into a second module (the one change that would make "the Safari path is unchanged" a reading exercise instead of a diff). Nothing that was on the browser path moved, and the evidence that it is unchanged is that App.test.tsx's tracking suite and the teleport, locate, check-in and PWA files all pass unedited. **`SampleTrackingState` is still thirteen members**, asserted twice — a type-level exhaustiveness check that stops a fourteenth compiling, and a runtime count of the object the hook returns under both drivers, so a member declared and left unproduced fails as well. **The four counters start at nought on every mount and are advanced only by a live `flush`.** The bridge's replayed payload seeds `newBars`, `visitUpdates`, `tooFastToReveal`, `queueDepth`, the internal `behindDepth`, `trackingActive` and `lastPosition`, and advances nothing: `useBarStamps` and `useVisits` read `version === 0` as "nothing has happened yet in this mount", so a counter moved by a replay would re-stamp bars discovered before the map existed every time the player returned to the map — a defect that is invisible in a browser, because a browser has no replay. That is asserted through the real map screen against a bridge that already holds a discovery and a mastering: nothing is stamped, `GET /api/bars` and `GET /api/fog` are each fetched once, no mastering toast appears, with the identical payload arriving live as the control that does all three. `postError` is `null` under the shell driver deliberately and permanently, and the panel's honest signal there is `behind` rising into `syncing` instead; `trackingActive` is the tracker's state and not the web view's visibility; `gpsStatus` reaches `poor` through staleness up to `GPS_STALE_MS` later than a browser reaches it through accuracy, which `ios/SPEC.md` 8.3 records as accepted and this release does not "fix". **Section 7.2 is amended in two places, and this block is what falsified both.** Its opening sentence said flatly that the client watches position and holds a wake lock; that is now true in a browser and false in the app, and the paragraph says so and points at `ios/SPEC.md` 8.3. Its closing sentence said a teleport is not a way to sample from two places at once; inside the app the native tracker keeps posting the real position while an admin is teleported, so the claim is scoped to this client and the two-poster consequence is stated with a pointer to `ios/SPEC.md` O-I8, which owns the decision not to fix it. Section 8.6 was checked and needed nothing: v1.59 already gave the third icon its four shell states and already scoped the "tracking pauses when the app is not in the foreground" sentence to the web app. Step D stays unticked with two of its five substeps built. Thirty-two tests, all web. Sections 7.2, 15; `packages/web/src/tracking/useSampleTracking.ts`, `packages/web/src/App.shelltracking.test.tsx`, `ios/SPEC.md` (the parent pin), `ios/PARENT-CONTRACT.md` (a new dependency entry D7, and one merge-record row), `HANDOVER.md`. 1769 tests (shared 243, api 596, web 697, tracker 233).
- **v1.61** — Row 2 of `ios/SPEC.md` Section 12's list for `main`, which completes that document's Step C. **`users.background_tracking_consented_at`** arrives by migration `004_users_background_tracking_consent.sql` — nullable, no default, so every account that already exists and every new registration reads as "has not consented" without a data migration, and withdrawal writes the same `NULL` back. Section 5.3 already specified the column at v1.59 and nothing implemented it; this is that gap closed rather than a new decision. **`PATCH /api/settings` takes the partial body** Section 9.2 has documented since v1.59: two optional booleans with a refine requiring at least one, an omitted key meaning unchanged. The route builds its `UPDATE` from the keys the body names, so a column the body did not name is not written at all — not read and written back, which is what a fixed two-placeholder statement would have forced — and both keys together are one `UPDATE` on one row, asserted by recording the SQL the route prepares rather than inferred from the two values that landed. It is a strict widening: every body that answered 200 before answers 200 now, and the two older tests pass unedited, which is the proof. The one body whose treatment changes is `{ isAnonymous: true, backgroundTracking: 5 }`, a 200 before and a 400 now. `{}` is a 400 and so, because the schema is deliberately not `.strict()`, is a body naming only an unknown key — a misspelt consent write is loud rather than a 200 that recorded nothing. **The `User` of Section 9.6 carries `backgroundTrackingConsentedAt` everywhere a user is returned**: register (the literal, `null` by construction), login, `GET /api/auth/me` and `PATCH /api/settings`'s own read-back are the four, and `routes/auth.ts` owns the row shape, the public shape and the mapper that all of them go through, so there was one place to change and no second copy to miss. `true` writes `Math.floor(Date.now() / 1000)`, the idiom `created_at` is already written with (Section 0, rule 6), and a repeated `true` re-stamps, because the record Article 7(1) wants is when the player last consented. The field is deliberately **absent** from the leaderboard entry, `GET /api/profile/:handle` and the admin user list, and each absence is asserted with the column set on the account in question, so it is the shape refusing to carry a value rather than the value happening to be `NULL`. `packages/web/src/api/types.ts`'s `User` gains the field and `updateSettings` in `client.ts` takes `{ isAnonymous?, backgroundTracking? }`; its one caller, the Settings screen's anonymity toggle, is unchanged. No response guard was added: there is no `isUser`, and Section 9.6's rule is that a response is validated only where a wrong shape renders as data — nothing on `main` reads this field. The twenty `User` fixtures across the web suite were filled in by hand for the same reason `rejected`'s were at v1.60: they are untyped wire bodies, so a green suite proves nothing about them. Twenty-three tests (api 19, web 4). Section 12's Phase 9 Step C is ticked, all four of its Definition-of-Done items now built across v1.60 and v1.61. Sections 5.3, 12, 15; `packages/api/migrations/004_users_background_tracking_consent.sql`, `packages/api/src/routes/auth.ts`, `packages/api/src/routes/account.ts`, `packages/web/src/api/types.ts`, `packages/web/src/api/client.ts`, `ios/SPEC.md` (the parent pin), `ios/PARENT-CONTRACT.md` (one merge-record row), `HANDOVER.md`. 1703 tests (shared 243, api 596, web 631, tracker 233).
- **v1.60** — Rows 1 and 11 of `ios/SPEC.md` Section 12's list for `main`, which between them unblock that document's Step E. **`POST /api/samples` and `POST /api/admin/teleport` answer with `rejected`** — `{ accuracy, future, stale, outsideCity, tooFast }`, one count per gate of Section 7.2 for this request's own samples, computed on the five `continue` branches of `processSampleBatch`'s loop and stored nowhere. Section 7.2's second step is two counts rather than one, because a phone whose clock runs fast and a phone whose network is down are the distinction the field exists to make. The teleport needed no code: it returns the pipeline's result verbatim, and all five of its counts are structurally zero — `TELEPORT_FIX` sets accuracy to 0, the synthetic sample's timestamp is the instant the clock gates measure against, an out-of-box point is refused with a 422 before the batch runs, and the speed guard is off — so 9.6's row for it, which said only that `tooFast` was always zero, is corrected and asserted. `packages/web/src/api/types.ts`'s `SamplesResponse` gains the field as required; `isSamplesResponse` deliberately does **not** check it, by O18's rule that a response is validated only where a wrong shape renders as data, and both the guard and its test now say so, so the next reader does not complete it. Nine tests: one per gate asserting that count is 1 and the other four 0, one that a sample above `FOG_MAX_SPEED_KMH` is refused a reveal and rejected by nothing, one mixed batch whose counts sum to the samples it did not accept (checked against a control user who posted the accepted sample alone), and two on the teleport. **`packages/api` becomes importable.** Row 11 as written — the devDependency and its lockfile entry on `packages/tracker` — could not have worked: the package declared no `main` and no `exports`, so `import { buildApp } from '@tipsytrails/api'` resolved to a directory with no entry point and failed with `ERR_MODULE_NOT_FOUND`, at resolution, where no `tsc --noEmit` looks. It gains `main`, `types` and an `exports` block pointing at `src/index.ts`'s build — the entry that exports `buildApp` and `loadEnv` and listens to nothing, `src/server.ts` being the one that listens — and `declaration: true`, without which `types` names a file that is never written and a typed import is an implicit `any` that fails `strict`. The root `prepare`, `pretypecheck` and `pretest` build `api` after `shared`, the same three places and the same reason `shared` already had, rather than a `pretest` inside `packages/tracker`: the ordering is `main`'s to own, and `typecheck` is where a typed import fails first. `packages/api/src/package-entry.test.ts` holds all of it — the self-reference by package name, and the resolution from `packages/tracker`'s own position in the workspace, which is the one the harness will perform — and it lives in `packages/api` because I7 gives `packages/tracker/` to the `ios-app` branch. Section 3's dependency table already named the devDependency; its commands table and build-order note did not. Phase 9's Step C stays unticked with one of its four items built. Sections 3, 9.6, 12, 15; `packages/api/src/routes/fog.ts`, `packages/api/package.json`, `packages/api/tsconfig.build.json`, `packages/tracker/package.json`, `package.json`, `pnpm-lock.yaml`, `packages/web/src/api/types.ts`, `packages/web/src/api/response-guards.ts`, `ios/SPEC.md` (the parent pin), `ios/PARENT-CONTRACT.md` (one merge-record row), `HANDOVER.md`. 1680 tests (shared 243, api 577, web 627, tracker 233).
...
- **v1.55** — Documentation only; no route, component, constant or test changed. `README.md` and `.env.example` are re-read against the code after the seven blocks that produced v1.46–v1.54, and three file lists in this document are completed. The README said the trusted-hop count behind the rate limits was still an open question (O10 closed in v1.46, and nothing is keyed on an address any more), that the seed holds 170 bars (it held 170 when that sentence was written on 2026-08-11, 180 after the widened import, and 179 since v1.49), that `data/db/` is absent from a fresh clone (the directory is committed as a bare `.gitkeep`), and that the city data is generated by "the three scripts in `scripts/`", where there are six and three of them generate it; it also did not carry the 2 GB swap requirement Section 4.3 has always said it carries, and said nothing about the rankings exclusion (v1.41) or the teleport (v1.41–v1.43). `.env.example` claimed the seeded admin has `must_change_password` set — it is 0 on purpose, Section 5.3 — and that `ADMIN_USER`/`ADMIN_PASSWORD` are "read once, on first boot", which stopped being the whole truth when `--rotate-password` landed in v1.44 and started ending sessions in v1.45. In this document, Section 4.2's seed-tree comment and Section 13.1's table both omitted `city.geojson` and `neighbours.geojson`, which `fetch-boundaries.ts` has written since the multi-city seam, and 13.1 had no row for `data/cities/<slug>.json`; `DATA-LICENSE` still named the three seed artefacts at their pre-seam flat paths and covered neither of the two boundary files. Sections 4.2, 13.1, 13.3; `README.md`, `.env.example`, `CONTRIBUTING.md`, `DATA-LICENSE`, `HANDOVER.md`. No open item changed status.
- **v1.54** — The badge floors stop being shipped to the browser. Section 0's rule 3 changes for the first time: constants now live in **two** modules and no third — `packages/shared/src/config.ts`, client-safe and bundled, and the new `packages/shared/src/server-config.ts`, holding only what a client may not be given. Six numbers moved, and nothing else: `CONFIG.BADGE_THRESHOLDS` became `SERVER_CONFIG.BADGE_THRESHOLDS`, `CONFIG.BADGE_KINDS` stayed behind as the array `['explorer', 'barfly']`, and the only file that reads a floor — `packages/api/src/badges.ts` — gained one import. No web module changed except the `BadgeKind` alias in `api/types.ts`, because `packages/web` never read a floor; it read the key set. Unreachability is enforced three ways rather than agreed: a `./server` subpath that `index.ts` does not re-export, an ESLint `no-restricted-imports` rule over `packages/web`, and `packages/web/src/bundle.test.ts`, which builds the production bundle on every `pnpm test` and fails on a badge period bound to a number. At v1.53 that bundle held `BADGE_THRESHOLDS:{explorer:{week:.1,...}}` verbatim; it now holds no such string and no such number. `packages/shared` still has zero runtime dependencies — `server-config.ts`'s only import is a type. The kinds and their floors are kept in step by `satisfies Record<BadgeKind, …>`, so a kind without floors, or floors without a kind, fails `pnpm typecheck`. Sections 0, 7.1, 7.7, 14; `CLAUDE.md`'s constants rule amended to match. O16 closed and removed, per Section 14's own delete-vs-keep rule — the question was answered and the answer lives in 7.1 and 7.7 — after two of its claims were found wrong: the fix touched none of the twelve imports it said it would, and its list of them named thirteen files, one of which (`screens/Leaderboard.tsx`) mentions `CONFIG` only in a comment.
...
- **v1.49** — `IMPORT_DUPLICATE_RADIUS_M` drops from 40 m to 15 m, and the committed seed is collapsed at last. The 40 m was tuned to merge Karlsruhe's two "Traube" records, which are not one venue mapped as a node and a building way but a restaurant and a beer garden on opposite sides of the street — so the old radius would have deleted a bar a player can walk into. Fettschmelze (two nodes 6.15 m apart, identical address) is the real duplicate and the only one in the city; any value between the two distances behaves identically, and 15 m is the round one. `scripts/collapse-seed-duplicates.ts` re-applies Section 11.1's rule to a committed `bars.json` offline, which is how `data/seed/karlsruhe/bars.json` went from 180 records to 179 without an Overpass round-trip. The import radius is now the tighter of the two duplicate radii, deliberately. Sections 4.2, 7.1, 11.1, 11.4, 14. O19 closed; O11's seed count corrected.
- **v1.48** — The 2D canvas fog fallback becomes visible, and its holes become one cell wide. `.fog-canvas-fallback` gains the rule it never had in `index.css` — absolutely positioned, `pointer-events: none`, and in a z-index band above the base map and below every mark the app draws over the map — without which MapLibre's own positioned canvas painted straight over an in-flow one and a device without WebGL2 saw no fog whatsoever. The bar markers state a z-index of their own so that band cannot cover the one control Section 7.5's check-in needs, and the picker's map becomes a stacking context so it cannot cover the picker's own controls. `estimateCellPixelSize` measures between the centres of cells (0, 0) and (1, 0), instead of from the grid's south-west boundary to the centre of cell 1 — one and a half cells, reported as one, so every hole was drawn about 1.5x too large. Section 7.3. O12 closed and removed; O13 narrowed to the interleaving that remains.
- **v1.47** — The two `by: 'username'` rate limits stop counting spellings instead of accounts. `resolveIdentity` trims and lowercases the submitted username before it becomes a bucket key, so it names the same account `users.username`'s `COLLATE NOCASE` does: `loginByUser` was 10 attempts per 5 minutes per *spelling*, which for a five-character name meant 32 buckets and 320 attempts against one account, growing exponentially with length; `resetByUser` had the same hole and additionally split ` admin` from `admin`, since the reset schemas trim after the limiter has already run. Normalised inside the limiter rather than at the two call sites, so a future username-keyed limit inherits it. Section 9.4.
- **v1.46** — The IP address stops being a rate-limit bucket key anywhere, and `by: 'ip'` is deleted from the union so that a limit which tried to use one would not compile. `auth` and `resetByIp` are gone; login is limited per submitted username (10 per 5 minutes), the three password-verifying routes share one deliberately generous global ceiling (`authGlobal`, 60/minute) that bounds argon2 work an unauthenticated caller can cause, and registration gets its own tighter global cap (`register`, 30/hour) and deliberately not `authGlobal` as well. The bug this fixes was site-wide: behind the tunnel every request arrived from the proxy, so ten login attempts a minute were ten for the whole site. `trustProxy` becomes `['loopback', 'linklocal', 'uniquelocal']` — no hop count, no vendor header — and now shapes nothing but Fastify's log lines. `POST /api/auth/change-password` stays unlimited, deliberately, and both exhaustion costs of the new design are written down rather than left to be discovered. O10 closed, by removing what it was waiting on. Sections 4.3, 7.1, 9.4, 9.5, 12 (Phase 1), 13.4, 14.
...
11. **`SUGGEST_DUPLICATE_RADIUS_M` and `IMPORT_DUPLICATE_RADIUS_M` stay separate** (v1.24; the import's became the smaller of the two in v1.49). Neither may be changed by changing the other, in either direction. Section 11.1.
...
14. **Karlsruhe's two "Traube" records are two venues, not one venue mapped twice** (v1.49 reversed v1.24's reading of them). `node/1447845917` and `way/54732961`, 25.34 m apart, same name, no address on either: a restaurant and a beer garden on opposite sides of the street. The OSM data looks exactly like a node-and-building double mapping and is not one, which is why `IMPORT_DUPLICATE_RADIUS_M` is 15 m and not 40 m — the wider radius, tuned to catch this pair, would delete a bar a player can walk into. Anyone widening it will meet the same records in the same data and re-derive the same wrong conclusion. Section 11.1.
15. **`POST /api/samples` reports the visits it expired** (v1.51 reversed a rule that had lived in `routes/fog.ts`'s comment since Phase 5). Do not re-derive it from `GET /api/visits/pending`, which reports none: that response is the pending list, where an expired visit is absent and the absence is the notice; the sample response is a list of changes, where absence means nothing happened to the visit. Silence there is what left an ended visit in the banner. Section 7.5.

---

_End of specification v1.65_
