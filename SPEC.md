...
- [x] App shell < 150 KB gzipped, excluding the map chunk; the map chunk (MapLibre + PMTiles, ~250 KB gzipped) is code-split and loaded only on map routes. A single bundle under 200 KB is not achievable with MapLibre and must not be attempted by trimming features. — measured from `pnpm --filter @tipsytrails/web build` on 2026-09-04: everything `index.html` loads eagerly (the entry chunk, the Rolldown runtime, the shared-package chunk, the cocktail-glass chunk and the stylesheet) is **82.7 kB gzipped in total**, and the MapLibre + PMTiles chunk is a further 239 kB gzipped, reached only through the `lazy(() => import(...))` route splits in `packages/web/src/App.tsx` — the map screen and the suggest screen's map picker, which is the one route beyond `/map` that shows a map. Nothing asserts either figure: no test in this repository measures a bundle size, so this is a reading of one build and not a budget that would fail if it were exceeded
...
- [~] Samples with accuracy > 200 m are discarded; so are stale and future-dated samples — accuracy: `packages/api/src/routes/fog.test.ts`: `discards a sample with accuracy worse than FOG_MAX_ACCURACY_M entirely` (`CONFIG.FOG_MAX_ACCURACY_M` is 200). Stale: `packages/api/src/routes/visit-samples.test.ts`: `a sample rejected as stale by Section 7.2 validation does not touch the visit`, which posts a sample older than `CONFIG.SAMPLE_MAX_AGE_MS` and gets `newCells: 0` back. **Future-dated is untested**: the guard exists — `packages/api/src/routes/fog.ts` skips a sample whose timestamp is further ahead of the server clock than `CONFIG.SAMPLE_MAX_CLOCK_SKEW_MS` — but no test anywhere posts a future-dated sample, so nothing would fail if that branch were removed
