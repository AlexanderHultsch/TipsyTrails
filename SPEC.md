...
- [ ] Step C — the server's two additions: `rejected` on `POST /api/samples` and the teleport, and the consent column behind the widened `PATCH /api/settings` — Definition of Done in `ios/SPEC.md` Section 12, Step C. **Half built at v1.60:** `rejected` is on both routes, with a test per gate and the mixed batch, and the web type carries it; the consent column and the partial `PATCH /api/settings` body are not built. The box stays unticked because Step C's Definition of Done is four items and one of them is done — a step is done when its Definition of Done is, and no earlier
