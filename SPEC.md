...
- [ ] Rate limits on auth endpoints return 429 when exceeded, **and no bucket anywhere is keyed on the client's address**: exhausting login for one username leaves every other username able to log in, whatever `X-Forwarded-For` the attempts carry (Section 9.4)
